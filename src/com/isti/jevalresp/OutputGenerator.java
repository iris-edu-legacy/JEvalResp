...
public class OutputGenerator
{
  ...
  public static final int UNIT_CONV_DEFIDX = 2;
    /**
     * Longer versions of strings associated with the '..._UNIT_CONV'
     * values.
     */
  public static final String [] UNIT_CONV_LONGSTRS =
                  { "Default", "Displacement", "Velocity", "Acceleration" };
    /** Value of 'pi' times 2. */
...
//  protected static final boolean XDEBUG_FLAG = true;   //true for messages
  ...
  {                     //setup number formatter:
     ...
     * @param respObj an 'edu.iris.Fissures.IfNetwork.Response' object
...
//    if(XDEBUG_FLAG)
//    {    //if extra debug messages enabled then initialize log file
//      LogFile.initGlobalLogObj(
//                          "JEvalResp.log",LogFile.ALL_MSGS,LogFile.NO_MSGS);
//    }
  }

    /**
     * Creates an output generating object.
     * @param cSpectraArray an array of complex spectra output values from
     * the calculated response.
     * @param calcFreqArray an array of frequency values for the calculated
     * response.
     * @param logSpacingFlag true to indicate that the frequency spacing
     * is logarithmic; false to indicate linear spacing.
     * @param useEstDelayFlag true to use estimated delay in phase
     * calculations on asymmetrical FIR filters.
     * @param showInputFlag true to show RESP input text (sent to stdout).
     */
  public OutputGenerator(ComplexBlk [] cSpectraArray,double [] calcFreqArray,
       boolean logSpacingFlag,boolean useEstDelayFlag,boolean showInputFlag)
  {
         //create array with single entry for all-stages response:
    respArraysInfoArray = new RespArraysInfo []
             { new RespArraysInfo(cSpectraArray,calcFreqArray,"Response") };
  ...
  public OutputGenerator(ComplexBlk [] cSpectraArray,
                             double [] calcFreqArray,boolean logSpacingFlag)
  {
    ...
    /**
     * Creates an output generating object.
     * @param ampPhaseArray an array of amplitude/phase response values.
     * @param calcFreqArray an array of frequency values for the calculated
     * response.
     * @param logSpacingFlag true to indicate that the frequency spacing
     * is logarithmic; false to indicate linear spacing.
  ...
  public OutputGenerator(AmpPhaseBlk [] ampPhaseArray,
                             double [] calcFreqArray,boolean logSpacingFlag,
                              boolean useEstDelayFlag,boolean showInputFlag)
  {
         //create array with single entry for all-stages response:
    respArraysInfoArray = new RespArraysInfo []
        { new RespArraysInfo(null,calcFreqArray,"Response",ampPhaseArray) };
    requestedFreqArray = calcFreqArray;
    this.logSpacingFlag = logSpacingFlag;
    this.useEstDelayFlag = useEstDelayFlag;
    this.showInputFlag = showInputFlag;
   ...
   * @param outputDirectory output directory or null for current directory.
   * @param fileName  String The system-dependent filename.
   * @throws IOException if the specified file is not found or if some other
   *                     I/O error occurs.
   * @return FileWriter object
   */
  public static FileWriter openFileWriter(
      File outputDirectory, String fileName) throws IOException
  {
     ...
     * 'getErrorMessage()') and returns false.  For the response to be seen
     * as valid, it must meet the following conditions: <pre>
     ...
     *    conversion must be 'Velocity' or 'Default'.</pre>
     * @param dontCheckUnitsFlag if false and the response contains multiple
     * stages then the input units must match the output units of previous
     * stages; if true then the units are not checked.
  ...
  public boolean checkResponse(boolean dontCheckUnitsFlag)
  {
    ...
    final int numStages;
    if((numStages=respObj.stages.length) <= 0)
    {
    ...
    {    //for each stage in response
      if((stageObj=respObj.stages[stageNum]) == null)
      {  //array element is null; set error message
        setErrorMessage("No data entered for stage #" + (stageNum+1) +
                                      " (out of " + numStages + " stages)");
        return false;
      }
      if(stageObj.type == null)
      {  //no type object; set error message
        setErrorMessage("No transfer type for stage #" + (stageNum+1));
        return false;
      }
      if(!dontCheckUnitsFlag && !polynomialFlag)
      {       //units checking enabled and no polynomial filters were found
        if(stageObj.input_units == null)
        {     //no input units object; set error message
          setErrorMessage("No input units for stage #" + (stageNum+1));
          return false;
        }
        if(stageObj.output_units == null)
        {     //no output units object; set error message
          setErrorMessage("No output units for stage #" + (stageNum+1));
          return false;
        }
        if(prevUnitObj != null && !stageObj.input_units.equals(prevUnitObj))
        {     //not the first stage and units do not match
          setErrorMessage("Input units (" +
                            RespUtils.unitToUnitImpl(stageObj.input_units) +
           ") for stage #" + (stageNum+1) + " do not match output units (" +
             RespUtils.unitToUnitImpl(prevUnitObj) + ") of previous stage");
          return false;
        }
        prevUnitObj = stageObj.output_units;     //set units for next stage
      }
         //check filters in stage:
      if((stageObj.filters == null || stageObj.filters.length <= 0) &&
                               !RespUtils.isGainObjValid(stageObj.the_gain))

      {  //no filters and no gain value for stage; set error message
        setErrorMessage("No filters in stage #" + (stageNum+1));
        return false;
      }
      if(stageObj.filters.length > 1)
      {  //too many filters; set error message
        setErrorMessage("More than one filter in stage #" + (stageNum+1));
        return false;
      }
      if(stageObj.filters != null && stageObj.filters.length > 0)
      {  //stage contains a filter object (not a gain-only stage)
        try        //check filter for stage:
        {
          if((filterObj=stageObj.filters[0]) == null)
          {     //no filter object; set error message
            setErrorMessage("Filter object in stage #" + (stageNum+1) +
                                                                " is null");
            return false;
          }
          ...
          {  //not polynomial type filter; check gain
            if(numStages > 1 && !RespUtils.isGainObjValid(stageObj.the_gain))
            {  //more than one stage and no gain value for stage; set message
              setErrorMessage("No gain value for stage #" + (stageNum+1) +
                                                " of multi-stage response");
              return false;
            }
          }
          if(filterObj.discriminator().equals(FilterType.POLEZERO))
          {     //poles/zeros type filter
            if(stageObj.the_normalization == null ||
                                   stageObj.the_normalization.length <= 0 ||
                                      stageObj.the_normalization[0] == null)
            {   //no normalization; set error message
              setErrorMessage("No normalization for poles/zeros " +
                                        "filter in stage #" + (stageNum+1));
              return false;
            }
            if(stageObj.type == TransferType.DIGITAL)
              needDeciFlag = true;       //decimation stage required
            else if(stageObj.type == TransferType.LAPLACE ||
                                       stageObj.type == TransferType.ANALOG)
            {
              needDeciFlag = false;      //decimation stage not required
            }
            else
            {    //invalid transfer type; set error message
              setErrorMessage("Invalid transfer type for poles/zeros " +
                                        "filter in stage #" + (stageNum+1));
              return false;
            }
          }
          else if(filterObj.discriminator().equals(FilterType.COEFFICIENT))
          {     //coefficients type filter
            if(stageObj.type == TransferType.DIGITAL)
              needDeciFlag = true;        //decimation stage required
            else
            {   //invalid transfer type; set error message
              setErrorMessage("Invalid transfer type for coefficients " +
                                        "filter in stage #" + (stageNum+1));
              return false;
            }
          }
          ...
          {     //response list filter
            if(stageNum > 0 || numStages > 1)
            {   //response list not only stage; set error message
              setErrorMessage("Other stages not allowed with Response " +
            ...
            if(stageObj.the_normalization != null &&
                                    stageObj.the_normalization.length > 0 &&
                                      stageObj.the_normalization[0] != null)
            {   //normalization not allowed; set error message
              setErrorMessage("Normalization not allowed with Response " +
                                          "List in stage #" + (stageNum+1));
              return false;
            }
...
//            }
            ...
            if((unitObj=listFilterObj.frequency_unit) != null &&
                                            unitObj.the_unit_base != null &&
                          (!unitObj.the_unit_base.equals(UnitBase.SECOND) ||
                                                    unitObj.exponent != -1))
            {   //frequency unit not Hertz (sec^-1); set error message
              setErrorMessage("Freq units (\"" +
                    RespUtils.unitToUnitImpl(unitObj) + "\") not 'Hertz' " +
                   "('Sec^-1') in Response List in stage #" + (stageNum+1));
              return false;
            }
            if((unitImplObj=RespUtils.unitToUnitImpl(
                                       listFilterObj.phase_unit)) != null &&
                                     !unitImplObj.equals(UnitImpl.DEGREE) &&
                                       !unitImplObj.equals(UnitImpl.RADIAN))
            {   //phase unit not degree or radian; set error message
              setErrorMessage("Phase units (\"" +
                  RespUtils.unitToUnitImpl(unitObj) + "\") not 'Degrees' " +
                 "or 'Radians' in Response List in stage #" + (stageNum+1));
              return false;
            }
            needDeciFlag = false;        //decimation stage not required
          }
            ...
            needDeciFlag = false;   //decimation stage not required
          if(needDeciFlag && (stageObj.the_decimation == null ||
                                      stageObj.the_decimation.length <= 0 ||
                                        stageObj.the_decimation[0] == null))
          {     //decimation required but not given; set error message
            setErrorMessage("Required decimation not found in stage #" +
                                                              (stageNum+1));
            return false;
          }
        }
        catch(Exception ex)
        {       //exception while processing (shouldn't happen); set message
          setErrorMessage("Error checking filter in stage #" +
                                                 (stageNum+1) + ":  " + ex);
          return false;
        }
      }
    }
    if(numStages == 1 && !polynomialFlag)
    {    //response contains single stage that is not polynomial filter;
         // stage must have sensitivity or gain
      if(!RespUtils.isSensObjValid(respObj.the_sensitivity))
      {       //no stage 0 sensitivity for response
        if((stageObj=respObj.stages[0]) != null &&
                               !RespUtils.isGainObjValid(stageObj.the_gain))
        {     //stage object OK and no gain value for stage; set message
          setErrorMessage("No 'stage 0' response sensitivity or gain " +
                                                  "value for single stage");
          return false;
        }
      }
    }
     ...
     *    a gain value.  If the response contains only one stage then
     *    either an overall (stage 0) sensitivity or a gain value for
     *    the stage (or both) must be specified.</pre>
     * @return true if response elements are valid
     */
  public boolean checkResponse()
  {
    ...
    /**
     * Normalizes the response and calculates the sensitivity for
     * all the stages in the response.  Normalization objects may be
     * added to the stages to hold the normalization values, and the
     * gain factor and frequency for stages may be modified.  The
     * calculated sensitivity value is retrievable via the
     * 'getCalcSensitivity()' method.  Note that this method assumes
     * that each stage contains only one filter object in its 'filters[]'
     * array.
     ...
     * (in which case 'getErorMessage()' may be used to fetch
  ...
  public boolean normalizeResponse(int startStageNum,int stopStageNum)
  {
    try
    {
      ...
      if(!RespUtils.isSensObjValid(respObj.the_sensitivity))
      {    //no sensitivity for response
        cSenseFreq = 0.0;         //put in default frequency value
        for(stageNum=0; stageNum<numStages; ++stageNum)
        {    //for each stage in response; find last gain with non-zero freq
          if(RespUtils.isGainObjValid(
                               gainObj=respObj.stages[stageNum].the_gain) &&
                                       !RespUtils.isZero(gainObj.frequency))
          {   //stage contains valid gain entry with non-zero frequency
...
//      }
      ...
      {  //stop value OK; decrement for 0-based array access
      ...
      if(startStageNum >= numStages)
      {  //no stages match; set error message
      ...
      {  //for each stage in desired range
//        if(XDEBUG_FLAG)
//        {     //send debug message to default log file
...
//        }
        ...
        if(stageObj != null && stageObj.filters != null &&
                                                stageObj.filters.length > 0)
        {     //stage contains filters (not gain-only stage)
              //get first filter object (we're assuming only one):
          if((filterObj=stageObj.filters[0]) == null)
          {     //no filter object; set error message
            setErrorMessage("Filter[0] of stage #" + (stageNum+1) +
                                                                " is null");
            return false;
          }
        }
        ...
        if(!RespUtils.isGainObjValid(gainObj=stageObj.the_gain))
        {     //stage does not contain a gain value
          if(filterObj == null ||
                   !filterObj.discriminator().equals(FilterType.POLYNOMIAL))
          {  //not a polynomial filter; check sensitivity/gain
            if(numStages > 1)
            {   //more than one stage in response; set error message
              setErrorMessage("No gain value for stage #" + (stageNum+1) +
                                                  " of multi-stage response");
              return false;
            }
            if(!RespUtils.isSensObjValid(respObj.the_sensitivity))
            {       //no stage 0 sensitivity for response; set error message
              setErrorMessage("No 'stage 0' response sensitivity or gain " +
                                                      "value for single stage");
              return false;
            }
          ...
          gainObj = stageObj.the_gain =
                        new Gain(respObj.the_sensitivity.sensitivity_factor,
        ...
        {     //stage contains filters (not gain-only stage)
          ...
          if(filterObj.discriminator().equals(FilterType.POLEZERO))
          {   //filter is poles/zeros type; get normalization
            if(stageObj.the_normalization == null ||
                                   stageObj.the_normalization.length <= 0 ||
          ...
          if(!filterObj.discriminator().equals(FilterType.POLYNOMIAL))
          {  //not a polynomial filter
            if((gainObj.frequency != cSenseFreq || (normObj != null &&
                                 normObj.normalization_freq != cSenseFreq)))
            {  //gain or normalization frequency needs to change
              if(filterObj.discriminator().equals(FilterType.POLEZERO))
              {  //poles/zeros type filter
                if(stageObj.type == TransferType.LAPLACE ||
                                       stageObj.type == TransferType.ANALOG)
                {  //analog poles/zeros filter
                  dfNum = analogTrans(filterObj.pole_zero_filter(),1.0,
                                    ((stageObj.type==TransferType.LAPLACE) ?
                             TWO_PI*gainObj.frequency : gainObj.frequency));
                  ofNum = analogTrans(filterObj.pole_zero_filter(),1.0,
                                    ((stageObj.type==TransferType.LAPLACE) ?
                                           TWO_PI*cSenseFreq : cSenseFreq));
                  if(RespUtils.isZero(dfNum) || RespUtils.isZero(ofNum))
                  {  //zero values returned from transformation; set message
                    setErrorMessage("Zero frequency in bandpass analog " +
                                        "filter in stage #" + (stageNum+1));
                    return false;
                  }
                }
                else if(stageObj.type == TransferType.DIGITAL)
                {  //digital poles/zeros filter; get decimation object
                  if(stageObj.the_decimation == null ||
                                        stageObj.the_decimation.length <= 0)
                  {  //decimation required but not given; set error message
                    setErrorMessage("Required decimation not found in " +
                                                  "stage #" + (stageNum+1));
                    return false;
                  }
                  if((sIntTimeObj=RespUtils.deciToSampIntTime(
                                       stageObj.the_decimation[0])) == null)
                  {  //unable to process decimation; set error message
                    setErrorMessage("Invalid decimation object in stage #" +
                                                              (stageNum+1));
                    return false;
                  }
                  dfNum = iirPzTrans(filterObj.pole_zero_filter(),1.0,
                        sIntTimeObj.doubleValue(),TWO_PI*gainObj.frequency);
                  ofNum = iirPzTrans(filterObj.pole_zero_filter(),1.0,
                                            sIntTimeObj.doubleValue(),wVal);
                }
                else
                {  //invalid transfer type; set error message
                  setErrorMessage("Invalid transfer type for poles/zeros " +
              ...
              {  //coefficients type filter
                ...
                if(coeffFilterObj.denominator.length <= 0)
                {  //no denominators, process as FIR filter
                  if(coeffFilterObj.numerator.length > 0)
                  {  //more than zero numerators
                    ...
                    dfNum = firTrans(coeffFilterObj,1.0,
                        sIntTimeObj.doubleValue(),TWO_PI*gainObj.frequency,
                                                                firTypeVal);
                    ofNum = firTrans(coeffFilterObj,1.0,
                                 sIntTimeObj.doubleValue(),wVal,firTypeVal);
                  }
                  else  //empty FIR filter; ignore it
                    dfNum = ofNum = null;     //setup to not change gain, etc
                }
                else
                {  //contains denominators, process as coefficients filter
                  dfNum = iirTrans(coeffFilterObj,1.0,
                        sIntTimeObj.doubleValue(),TWO_PI*gainObj.frequency);
                  ofNum = iirTrans(coeffFilterObj,1.0,
                                            sIntTimeObj.doubleValue(),wVal);
                }
              }
              ...
              if(dfNum != null && ofNum != null)
              {  //values were entered; process them
                ofSqVal = Math.sqrt(ofNum.real*ofNum.real +
                                                     ofNum.imag*ofNum.imag);
                     //put in new gain factor and frequency:
                newGainVal = (double)gainObj.gain_factor / Math.sqrt(
                            dfNum.real*dfNum.real + dfNum.imag*dfNum.imag) *
                                                                    ofSqVal;
                gainObj.gain_factor = (float)newGainVal;
                gainObj.frequency = (float)cSenseFreq;
                if(normObj != null)
                {  //normalization object already exists; modify it
                  normObj.ao_normalization_factor = (float)(1.0 / ofSqVal);
                  normObj.normalization_freq = (float)cSenseFreq;
                }
                else
                {  //normalization object does not yet exist; create one
                  normObj = new Normalization((float)(1.0/ofSqVal),
                                                         (float)cSenseFreq);
      ...
      if(numStages == 1 && startStageNum < 1)
      {     //single stage response and stage wasn't skipped
                 //enter calc sensitivity as overall response sensitivity:
        if(respObj.the_sensitivity != null)
        {   //already contains sensitivity object; modify it
          respObj.the_sensitivity.sensitivity_factor = (float)calcSensArr[0];
          respObj.the_sensitivity.frequency = (float)cSenseFreq;
        }
        else
        {   //no previous sensitivity object; create and enter new one
          respObj.the_sensitivity = new Sensitivity((float)calcSensArr[0],
                                                       (float)cSenseFreq);
        }
      }
    ...
    {         //some kind of error occurred; set error message
     ...
     * containing the generated complex spectra response values may be
     * fetched via the 'getCSpectraArray()' method, and the amplitude/phase
     * response values may be calculated and returned via the
     * 'getAmpPhaseArray()' method.  If 'normalizeResponse()' has not yet
     * been called then it is called first.  Note that this method assumes
     * that each stage contains only one filter object in its 'filters[]'
     * array.
     * @param freqArray an array of frequency values to use.
     * @param logSpacingFlag true to indicate that the frequency spacing
     * is logarithmic; false to indicate linear spacing.
     * @param outUnitsConv output units conversion value for the requested
  ...
  public boolean calculateResponse(double [] freqArray,
                  boolean logSpacingFlag,int outUnitsConv,int startStageNum,
             int stopStageNum,boolean useEstDelayFlag,boolean showInputFlag,
                                   ...
                                   boolean totalSensitFlag,double b62XValue)
  {
    ...
    if(!normalizedFlag)
    {    //not yet normalized; do it now
      if(!normalizeResponse(startStageNum,stopStageNum))
    ...
    try
    {
      ...
      if(numStages > 0 && respObj.stages[0] != null &&
                                        respObj.stages[0].filters != null &&
                                     respObj.stages[0].filters.length > 0 &&
                         (filterObj=respObj.stages[0].filters[0]) != null &&
                          filterObj.discriminator().equals(FilterType.LIST))
      {  //list filter; use its frequency array instead of parameter
        lsFilterObj = filterObj.list_filter();   //set handle to filter
        freqArray = RespUtils.floatToDoubleArray(lsFilterObj.frequency);
        if(lsFilterObj.amplitude == null ||
                          lsFilterObj.amplitude.length < freqArray.length ||
                                                lsFilterObj.phase == null ||
                                lsFilterObj.phase.length < freqArray.length)
        {     //amp or phase array not large enough; set error message
          setErrorMessage("Amp or phase array too small in Response " +
                                                             " List stage");
          return false;
        }
        if(listInterpInFlag)
        {     //interpolate amp/phase values input from List blockette
                   //use req freqs, with any out-of-range freqs clipped:
          final double [] newFreqArray =
                           clipFreqArray(requestedFreqArray,freqArray,true);
          if(newFreqArray.length <= 0)
          {   //all requested frequencies were clipped
            setErrorMessage("Error interpolating values in Response " +
                      "List stage:  All requested freqencies out of range");
            return false;
          }
          final CubicSpline splineObj = new CubicSpline();
                   //interpolate List blockette amplitude values:
          if((listInterpAmpArr=splineObj.calcSpline(freqArray,
                        RespUtils.floatToDoubleArray(lsFilterObj.amplitude),
                               listInterpTension,1.0,newFreqArray)) == null)
          {   //error interploating values
            setErrorMessage("Error interpolating amplitude values in " +
                                                  "Response List stage:  " +
          ...
          if((listInterpPhaseArr=splineObj.calcSpline(
                              freqArray,unwrappedPhaseArr,listInterpTension,
                                                 1.0,newFreqArray)) == null)
          {   //error interploating values
            setErrorMessage("Error interpolating phase values in " +
                                                  "Response List stage:  " +
                                         splineObj.getErrorMessageString());
            return false;
          }
        ...
        UnitImpl unitImplObj;
        if((unitImplObj=RespUtils.unitToUnitImpl(lsFilterObj.phase_unit)) !=
                                                                       null)
        {     //phase units object exists
          ...
          {   //unknown phase units type; set error message
            setErrorMessage("Invalid phase units type (\"" +
                                unitImplObj + "\") in Response List stage");
            return false;
          }
        }
      ...
      int inpUnitsConv;
      if(outUnitsConv >= 0 && outUnitsConv != DEFAULT_UNIT_CONV &&
                                                            numStages > 0 &&
         (inpUnitsConv=toUnitConvIndex(respObj.stages[0].input_units)) >= 0)
      {  //not default units, >0 stages and input units conversion factor OK
              //use negated power of first input unit for unit scale factor:
        unitScaleFact = RespUtils.pow10(
                -RespUtils.toFirstUnitPower(respObj.stages[0].input_units));
      }
      else
      {  //default units or zero stages in response
      ...
      final RespArraysInfo [] rArrsInfoArr =
                                            new RespArraysInfo[numStages+1];
              //enter 'RespArraysInfo' object for all-stages response:
      rArrsInfoArr[0] = new RespArraysInfo(numFreq,freqArray,"Response");
      String typeStr;
      for(stageNum=startStageNum; stageNum<=stopStageNum; ++stageNum)
      {  //for each stage to be processed; get type ID for stage
        if((typeStr=getTypeStrForStage(respObj.stages[stageNum])) != null &&
                                                       typeStr.length() > 0)
        {     //type ID string contains data
          typeStr = " " + typeStr;     //prepend space to ID string
        }
                   //enter 'RespArraysInfo' object into array:
        rArrsInfoArr[stageNum+1] = new RespArraysInfo(
                           numFreq,freqArray,("Stg "+(stageNum+1)+typeStr));
      }
      ...
      lastUnitProc = respObj.stages[stopStageNum].output_units;
         ...
         // units conversion must be Velocity or Default:
      if((RespUtils.PASCAL_UNITIMPL_OBJ.equals(firstUnitProc) ||
                       RespUtils.TESLA_UNITIMPL_OBJ.equals(firstUnitProc) ||
                 RespUtils.CENTIGRADE_UNITIMPL_OBJ.equals(firstUnitProc)) &&
                                  this.outUnitsConv != VELOCITY_UNIT_CONV &&
                                     this.outUnitsConv != DEFAULT_UNIT_CONV)
      {
        setErrorMessage("Input units \"" +
                                   RespUtils.unitToUnitImpl(firstUnitProc) +
                                                  "\" not allowed with \"" +
                                  getLongUnitConvString(this.outUnitsConv) +
      ...
      boolean evalFlag;
      for(int fIdx=0; fIdx<numFreq; ++fIdx)
...
//          }
          stageObj = respObj.stages[stageNum];
          if(stageObj.filters != null && stageObj.filters.length > 0)
          {   //stage contains filters (not a gain-only stage)
            if(stageObj.the_normalization != null &&
                                    stageObj.the_normalization.length > 0 &&
                                      stageObj.the_normalization[0] != null)
            {   //stage contains normalization; save value
              normFact =
            ...
            if((filterObj=stageObj.filters[0]) == null)
            {   //no filter object; set error message
              setErrorMessage("Filter object for stage #" + (stageNum+1) +
                                                                " is null");
              return false;
            }
            ...
            {   //poles/zeros type filter
              ...
              if(stageObj.type == TransferType.LAPLACE ||
                                       stageObj.type == TransferType.ANALOG)
              {      //analog poles/zeros filter
                ofNum = analogTrans(pzFilterObj,normFact,
                                    ((stageObj.type==TransferType.LAPLACE) ?
                                                 TWO_PI*freqVal : freqVal));
                evalFlag = true;         //indicate evaluation performed
              }
              else if(stageObj.type == TransferType.DIGITAL)
              {      //digital poles/zeros filter
                if(pzFilterObj.poles.length > 0 ||
                                               pzFilterObj.zeros.length > 0)
                {    //filter is not empty
                  if(stageObj.the_decimation == null ||
                                        stageObj.the_decimation.length <= 0)
                  {  //decimation required but not given; set error message
                  ...
                  ofNum = iirPzTrans(pzFilterObj,normFact,
                                            sIntTimeObj.doubleValue(),wVal);
                  evalFlag = true;       //indicate evaluation performed
                }
              }
            ...
            {   //coefficients type filter
              ...
              {      //no denominators, process as FIR filter
                ...
                {    //more than zero numerators
                             //determine if asymmetrical or symmetrical FIR:
                  final int firTypeVal = determineFirTypeVal(filterObj);
                  ofNum = firTrans(coeffFilterObj,normFact,
                                 sIntTimeObj.doubleValue(),wVal,firTypeVal);
                  if(firTypeVal == FIR_ASYM)
                  {  //asymmetric FIR; requires delay correction
                    if(useEstDelayFlag)
                    {  //using estimated delay in phase calc correction
                      if((tDoubleObj=RespUtils.quantityToIntTime(
                             stageObj.the_decimation[0].estimated_delay)) !=
                                                                       null)
                      {  //estimated-delay time value fetched OK
                        deltaVal = tDoubleObj.doubleValue();
                                  //factor in estimated-delay value:
                        ofNum.zMultiply(Math.cos(wVal*deltaVal),
                                                   Math.sin(wVal*deltaVal));
                      }
                    }
                    else
                    {  //not using estimated delay in phase calc correction
                      if((tDoubleObj=RespUtils.quantityToIntTime(
                          stageObj.the_decimation[0].correction_applied)) !=
                                                                       null)
                      {  //correction-applied time value fetched OK
                             //calculate "delta = corrApplied - calcDelay":
                        deltaVal = tDoubleObj.doubleValue() - ((((double)
                                   (coeffFilterObj.numerator.length-1))/2) *
                                                 sIntTimeObj.doubleValue());
              ...
              else if(coeffFilterObj.numerator.length > 0)
              {      //process as coefficients filter
                ofNum = iirTrans(coeffFilterObj,normFact,
                                            sIntTimeObj.doubleValue(),wVal);
                evalFlag = true;         //indicate evaluation performed
              }
            }
            ...
            {   //response list filter
              ...
              if(listInterpInFlag)
              {    //using interpolated amp/phase values
              ...
              {    //using amp/phase values from List blockette
              ...
              ofNum = new ComplexBlk(ampVal*Math.cos(phaseVal),
                                                 ampVal*Math.sin(phaseVal));
              evalFlag = true;           //indicate evaluation performed
            }
            else if(filterObj.discriminator().equals(FilterType.POLYNOMIAL))
            {   //polynomial type filter
              if(b62XValue <= 0.0)
              {  //b62_x value not positive
                setErrorMessage("Valid 'b62_x' value must be specified " +
                                                 "for polynomial response");
                return false;
              }
              ampVal = 0.0;       //initialize amplitude value
                        //get array of coefficient and error values:
              final CoefficientErrored [] coeffsArr =
                             filterObj.polynomial_filter().coeff_err_values;
              ...
              {  //array not empty
                ...
                {
                   ampVal += coeffsArr[j].value * j *
                                                    Math.pow(b62XValue,j-1);
                }
        ...
        final float totalSensitVal = (totalSensitFlag &&
                                          respObj.the_sensitivity != null) ?
                    respObj.the_sensitivity.sensitivity_factor : (float)1.0;
        double calcSensVal;
              //process conversions for each stage entry:
        for(stageNum=0; stageNum<=numStages; ++stageNum)
        {     //for each possible stage entry
          if((rArrInfoObj=rArrsInfoArr[stageNum]) != null &&
                            (ofNum=rArrInfoObj.cSpectraArray[fIdx]) != null)
          {   //stage entry contains data
            if(totalSensitFlag)
            {  //using stage 0 (total) sensitivity
            ...
            {  //using computed sensitivity from each stage
                   //get calculated sensitivity value for stage index:
              calcSensVal = (calcSensitivityArray != null &&
                                   stageNum < calcSensitivityArray.length) ?
                                       calcSensitivityArray[stageNum] : 1.0;
                   //multiply in sensitivity and unit conv scale factor:
              ofNum.real *= calcSensVal * unitScaleFact;
              ofNum.imag *= calcSensVal * unitScaleFact;
            }
                   //handle any units conversions:
            if(outUnitsConv != inpUnitsConv)
            {      //requested output units different from input units
              if(inpUnitsConv == DISPLACE_UNIT_CONV)
              {    //input unit is 'displacement'; convert to 'velocity'
                if(wVal != 0.0)
              ...
              if(outUnitsConv == DISPLACE_UNIT_CONV)
                ofNum.zMultiply(0.0,wVal);
              else if(outUnitsConv == ACCEL_UNIT_CONV)
              {    //requested output is 'acceleration'; convert it
                if(wVal != 0.0)
                  ofNum.zMultiply(0.0,-1.0/wVal);
                else
                  ofNum.real = ofNum.imag = 0.0;
  ...
  {
    return calculateResponse(freqArray,logSpacingFlag,outUnitsConv,
                   startStageNum,stopStageNum,useEstDelayFlag,showInputFlag,
             ...
             int stopStageNum,boolean useEstDelayFlag,boolean showInputFlag)
  {
     ...
     */
  public boolean calculateResponse(double [] freqArray,
                                    boolean logSpacingFlag,int outUnitsConv,
                                         int startStageNum,int stopStageNum)
  {
    return calculateResponse(freqArray,logSpacingFlag,outUnitsConv,
                                    startStageNum,stopStageNum,false,false);
  }
     ...
     * that each stage contains only one filter object in its 'filters[]'
     * array.
     * @param freqVal a single frequency value to use.
     * @param outUnitsConv output units conversion value for the requested
     * output units type; one of the '..._UNIT_CONV' values.
     * @param startStageNum if greater than zero then the start of the
     * range of stage sequence numbers to use, otherwise all stages
  ...
  public boolean calculateResponse(double freqVal,int outUnitsConv,
                 int startStageNum,int stopStageNum,boolean useEstDelayFlag,
                                                      boolean showInputFlag)
  {
                        //create array with single frequency value:
    return calculateResponse((new double [] {freqVal}),true,outUnitsConv,
                  startStageNum,stopStageNum,useEstDelayFlag,showInputFlag);
  }
                                         ...
                                         int startStageNum,int stopStageNum)
  {
    return calculateResponse(
               freqVal,outUnitsConv,startStageNum,stopStageNum,false,false);
  }
  ...
  {
    return (respArraysInfoArray != null && respArraysInfoArray.length > 0 &&
                                           respArraysInfoArray[0] != null) ?
                                respArraysInfoArray[0].cSpectraArray : null;
  }

  /**
   * Calculates amplitude/phase values via the complex-spectra values
   * from the given reponse arrays/information object.
                                                             ...
                                                             int respArrIdx)
  {
    ...
    try
    {
      ...
      double real,imag;
      while(i < len)
      {  //for each element in complex spectra array, convert to amp/phase
        real = rArrsInfoObj.cSpectraArray[i].real;
        imag = rArrsInfoObj.cSpectraArray[i].imag;
        ampPhaseArr[i] = new AmpPhaseBlk(Math.sqrt(real*real+imag*imag),
                            Math.atan2(imag,real+1.0e-200)*180.0/Math.PI);
        ++i;
      }
    }
    catch(Exception ex)
    {    //some kind of error; set error message
      setErrorMessage("Exception error calculating amp/phase value" +
            ((rArrsInfoObj != null) ? (" for " + rArrsInfoObj.identifyStr) :
                      UtilFns.EMPTY_STRING) + " (index=" + i + "):  " + ex);
      return false;
    }
    if(listInterpOutFlag)
    {  //interpolate amp/phase values generated via List blockette
      try
      {            //use req freqs, with any out-of-range freqs clipped
                   // (if all-stages entry then show "Note:" messages):
        final double [] newFreqArray = clipFreqArray(requestedFreqArray,
                               rArrsInfoObj.frequencyArr,(respArrIdx == 0));
        if(newFreqArray.length <= 0)
        {   //all requested frequencies were clipped
          setErrorMessage("Error interpolating amp/phase output values:  " +
                                   "All requested freqencies out of range");
          return false;
        }
        final CubicSpline splineObj = new CubicSpline();
        final double [] listInterpAmpArr;
                   //interpolate List blockette amplitude values:
        if((listInterpAmpArr=splineObj.calcSpline(
                 rArrsInfoObj.frequencyArr,fetchAmpPhaAmpArray(ampPhaseArr),
                               listInterpTension,1.0,newFreqArray)) == null)
        {     //error interploating values
          setErrorMessage("Error interpolating amplitude output values:  " +
                                         splineObj.getErrorMessageString());
          return false;
        }
        ...
        double [] listInterpPhaseArr;
                   //interpolate List blockette phase values:
        if((listInterpPhaseArr=splineObj.calcSpline(
                                rArrsInfoObj.frequencyArr,unwrappedPhaseArr,
                               listInterpTension,1.0,newFreqArray)) == null)
        {     //error interploating values
          setErrorMessage("Error interpolating phase output values:  " +
                                         splineObj.getErrorMessageString());
          return false;
        }
      ...
      {    //some kind of error; set error message
        setErrorMessage(
                   "Exception error interpolating amp/phase output values" +
            ((rArrsInfoObj != null) ? (" for " + rArrsInfoObj.identifyStr) :
                                        UtilFns.EMPTY_STRING) + ":  " + ex);
        return false;
      }
    }
    if(unwrapPhaseFlag)
    {  //flag set for unwrapping phase values (via "-unwrap" parameter)
      try
      {                                //get generated phase data:
        final double [] srcPhaseArr = fetchAmpPhaPhaseArray(ampPhaseArr);
        ...
        if(unwrappedPhaseArr != srcPhaseArr)
        {  //phase data changed; enter with new phase data
          rArrsInfoObj.setAmpPhaseArray(fetchAmpPhaAmpArray(ampPhaseArr),
                                                         unwrappedPhaseArr);
          return true;
        }
      }
      ...
      {  //some kind of error; set error message
        setErrorMessage("Exception error unwrapping phase output values" +
            ((rArrsInfoObj != null) ? (" for " + rArrsInfoObj.identifyStr) :
                                        UtilFns.EMPTY_STRING) + ":  " + ex);
        return false;
      }
    ...
    /**
     * Returns the array of amplitude/phase response
     * values generated by 'calculateResponse()'.
     * @return The array of amplitude/phase response
     * values generated by 'calculateResponse()', or null if
     * 'calculateResponse()' has not yet been performed or if
     * an error occurs (in which case an error message may be
  ...
  public AmpPhaseBlk [] getAmpPhaseArray()
  {
    ...
    if(respArraysInfoArray != null && respArraysInfoArray.length > 0 &&
                                             respArraysInfoArray[0] != null)
    {    //array of response arrays/info objs contains at least one entry
      ...
      if(respArraysInfoArray[0].cSpectraArray != null)
      {  //complex spectra array exists; calc amp/phase vals for all stages
   ...
   * @return true if the amp/phase array has been calculated; false
   * if not.
   */
  public boolean isAmpPhaseArrayCalculated()
  {
    return (respArraysInfoArray != null && respArraysInfoArray.length > 0 &&
                                           respArraysInfoArray[0] != null &&
                              respArraysInfoArray[0].ampPhaseArray != null);
  }

    /**
     * Returns the arrays of amplitude/phase response values generated
     * by 'calculateResponse()' for all stages in the response.
     * @return An array of 'RespArraysInfo' objects whose amplitude/phase
     * values have been calculated and entered, or null if
     * 'calculateResponse()' has not yet been performed or if
     * an error occurs (in which case an error message may be
     * fetched via the 'getErrorMessage()' method).
     */
  public RespArraysInfo [] getAllStagesAmpPhaseArrays()
  {
    ...
    {    //array of response arrays/info objs contains at least one entry
      ...
      if(respArraysInfoArray[0].ampPhaseArray != null)
      {  //amp/phase array has been created
      ...
      if(respArraysInfoArray[0].cSpectraArray != null)
      {  //complex spectra array exists
              //if amp/phase array not yet created then do it now:
        if(!ampPhaseArrCreatedFlag)
          calcAmpPhaseArray(respArraysInfoArray[0],0);
        if(!allStagesAmpPhaseCalcFlag)
        {     //amp/phase values for rest of stages not yet calculated
          allStagesAmpPhaseCalcFlag = true;      //indicate calculated
          for(int i=1; i<respArraysInfoArray.length; ++i)
          {   //for each remaining stage entry in array
  ...
  {
    if(!anyAmpsNotPosSetupFlag)
    {    //'anyAmpsNotPositiveFlag' flag not yet setup
      try
      {
        final AmpPhaseBlk [] ampPhaseArr;
        if((ampPhaseArr=getAmpPhaseArray()) != null) //calc & fetch A/P data
        {     //amplitude-phase array fetched OK
          for(int i=0; i<ampPhaseArr.length; ++i)
          {        //for each amplitude value
            if(ampPhaseArr[i].amp <= 0.0)
      ...
      {  //some kind of exception error; setup to return false
    ...
    /**
     * Determines, for all stages in the response, if any amplitude values
     * are less than or equal to zero.
     * @return true if, for all stages in the response, any amplitude
     * values are less than or equal to zero.
     */
  public boolean allStagesAnyAmpsNotPositive()
  {
    if(!allStagesAnyAmpsNotPosSetupFlag)
    {    //'allStagesAnyAmpsNotPositiveFlag' flag not yet setup
      try
      {
        if(respArraysInfoArray != null && respArraysInfoArray.length > 0 &&
                                           respArraysInfoArray[0] != null &&
                              ...
                              respArraysInfoArray[0].ampPhaseArray != null))
        {
          ...
          outerLoop:
          for(int stgIdx=0; stgIdx<respArraysInfoArray.length; ++stgIdx)
          {   //for each response/stage entry in array
            if(respArraysInfoArray[stgIdx] != null &&
                  (ampPhaseArr=respArraysInfoArray[stgIdx].ampPhaseArray) !=
                                                                       null)
            {     //amplitude-phase array fetched OK
              for(int i=0; i<ampPhaseArr.length; ++i)
              {    //for each amplitude value
    ...
    /**
     * Calculates the response's sample interval, delay and correction
     * applied values.  This is method is called automatically the
     * first time any of the values are requested.
     */
  protected void calcIntvlDelayCorrValues()
  {
    ...
    sampleInterval = estimatedDelay = correctionApplied =
                                                      calculatedDelay = 0.0;
...
//        }
        if(stageObj.the_decimation != null &&
                                       stageObj.the_decimation.length > 0 &&
                               (deciObj=stageObj.the_decimation[0]) != null)
        {     //stage has a valid Decimation object
//          if(XDEBUG_FLAG)
//          {        //send debug message to default log file
//            LogFile.getGlobalLogObj().debug("  Stage contains decimation");
//          }
          if((doubleObj=RespUtils.deciToSampIntTime(deciObj)) != null)
          {   //sample interval calculated OK
...
//            }
          ...
          if((doubleObj=RespUtils.quantityToIntTime(
                                          deciObj.estimated_delay)) != null)
          {        //value fetched OK
...
//            }
          ...
          if((doubleObj=RespUtils.quantityToIntTime(
                                       deciObj.correction_applied)) != null)
          {   //value fetched OK
...
//            }
          ...
          if(stageObj.filters != null && stageObj.filters.length > 0 &&
                                  (filterObj=stageObj.filters[0]) != null &&
                   filterObj.discriminator().equals(FilterType.COEFFICIENT))
          {   //stage has coefficients filter
//            if(XDEBUG_FLAG)
//            {      //send debug message to default log file
//              LogFile.getGlobalLogObj().debug(
...
//            }
            ...
            if(coeffFilterObj.denominator.length <= 0 &&
                                        coeffFilterObj.numerator.length > 0)
            {  //no denominators (FIR filter) and at least one numerator
              calculatedDelay +=       //calc and add to calculated delay
                         (((double)(coeffFilterObj.numerator.length-1))/2) *
                                                               stageSampInt;
//              if(XDEBUG_FLAG)
//              {      //send debug message to default log file
//                LogFile.getGlobalLogObj().debug(
...
//              }
  ...
  public double getCalculatedDelay()
  {
    ...
    /**
     * Writes the complex spectra response values generated by
     * 'calculateResponse()' to the given output stream.
     * Each outputted line contains 3 numbers:  the frequency followed by
     * the real and imaginary parts of the complex spectra response value
     * for the frequency.  Each number is written in floating-point
     * exponent format with 7 significant digits.
     * @param outStm output stream to write to.
     * @param fName file name associated with output stream.
     * @param headerStr a String of header information to be included in
     * the output, or null to indicate no header information.
     * @return true if successful, false if error (in which case an error
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeCSpectraData(Writer outStm,String fName,
                                                           String headerStr)
  {
    final ComplexBlk [] cSpectraArr;
    if((cSpectraArr=getCSpectraArray()) == null)
    {    //no complex spectra array; set error message
      setErrorMessage("Method 'calculateResponse()' not yet performed");
      return false;
    }
    try
    {
      final PrintWriter out = new PrintWriter(new BufferedWriter(outStm));
      if(headerStr != null)
      {  //header string was provided
        out.println(headerStr);                  //show provided header info
        out.println(getDescHeaderString());      //add frequency info
        out.println(Run.HDR_CMT_STR.trim());     //add blank line
                                                 //add column headers:
        out.println(Run.HDR_CMT_STR + "  Freq        Real       Imaginary");
      }
      final double [] freqArr = getCalcFreqArray();
      final int len = freqArr.length;
      for(int i=0; i<len; ++i)
      {  //for each entry in arrays; write line of output
        out.println(RespUtils.fmtNumber(freqArr[i]) + " " +
                            RespUtils.fmtNumber(cSpectraArr[i].real) + " " +
                                  RespUtils.fmtNumber(cSpectraArr[i].imag));
      }
      if(!out.checkError())       //if no stream errors flagged then
        return true;              //return OK flag
         //stream error flagged; set error message
      setErrorMessage("Stream error writing to output (filename=\"" +
                                                              fName + "\"");
    }
    catch(Exception ex)
    {    //exception error occurred; set error message
      setErrorMessage("Error writing to output (filename=\"" + fName +
                                                             "\"):  " + ex);
    }
     ...
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeCSpectraData(Writer outStm,String headerStr)
  {
    return writeCSpectraData(outStm,"(stream)",headerStr);
  }

    /**
     * Writes the complex spectra response values generated by
     * 'calculateResponse()' to a file with the given name.
     * Each outputted line contains 3 numbers:  the frequency followed by
     * the real and imaginary parts of the complex spectra response value
     * for the frequency.  Each number is written in floating-point
     * exponent format with 7 significant digits.
     ...
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeCSpectraData(
      File outputDirectory,String fName,String headerStr)
  {
    final FileWriter out;
    try
    {         //open file for output:
      out = openFileWriter(outputDirectory,fName);
    }
    catch(Exception ex)
    {         //error opening file; set error message
      setErrorMessage("Error opening file (\"" + fName +
                                                  "\") for output:  " + ex);
      return false;
    }
    final boolean retFlag = writeCSpectraData(out,fName,headerStr);
    try { out.close(); }          //close output file
    catch(IOException ex) {}
    return retFlag;
  }

    /**
     * Calculates and writes amplitude/phase response values generated
     * by 'calculateResponse()' to the two given output streams.
     * Each outputted line contains 2 numbers:  the frequency followed by
     * the amplitude or phase response value for the frequency.  Each
     * number is written in floating-point exponent format with 7
     * significant digits.
     * @param ampOutStm output stream to write amplitude data to.
     * @param phaseOutStm output stream to write phase data to.
     * @param ampFName file name associated with the amplitude data
     * output stream.
     * @param phaseFName file name associated with the phase data
     * output stream.
     * @param headerStr a String of header information to be included in
     * the output, or null to indicate no header information.
     * @return true if successful, false if error (in which case an error
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeAmpPhaseData(Writer ampOutStm,Writer phaseOutStm,
                         String ampFName,String phaseFName,String headerStr)
  {
    final AmpPhaseBlk [] ampPhaseArr;
    if((ampPhaseArr=getAmpPhaseArray()) == null) //calc & fetch A/P data
    ...
    try
    {
      final double [] freqArr = getCalcFreqArray();
      final int len = freqArr.length;
      PrintWriter out = new PrintWriter(new BufferedWriter(ampOutStm));
      String freqHdrStr = "";
      if(headerStr != null)
      {  //header string was provided
        out.println(headerStr);                  //show provided header info
        out.println(freqHdrStr=getDescHeaderString());  //add frequency info
        out.println(Run.HDR_CMT_STR.trim());     //add blank line
                                                 //add column header:
        out.println(Run.HDR_CMT_STR + "  Freq      Amplitude");
      }
      for(int i=0; i<len; ++i)
      {  //for each entry in arrays; write line of output
        out.println(RespUtils.fmtNumber(freqArr[i]) + " " +
                                   RespUtils.fmtNumber(ampPhaseArr[i].amp));
      }
      if(!out.checkError())
      {  //no stream errors flagged
        out = new PrintWriter(new BufferedWriter(phaseOutStm));
        if(headerStr != null)
        {     //header string was provided
          out.println(headerStr);                //show provided header info
          out.println(freqHdrStr);               //add frequency info
          out.println(Run.HDR_CMT_STR.trim());   //add blank line
                                                 //add column header:
          out.println(Run.HDR_CMT_STR + "  Freq        Phase");
        }
        for(int i=0; i<len; ++i)
        {  //for each entry in arrays; write line of output
          out.println(RespUtils.fmtNumber(freqArr[i]) + " " +
                                 RespUtils.fmtNumber(ampPhaseArr[i].phase));
        }
        if(!out.checkError())     //if no stream errors flagged then
          return true;            //return OK flag
              //stream error flagged; set error message
        setErrorMessage("Stream error writing phase output (filename=\"" +
                                                         phaseFName + "\"");
      }
      else
      {       //stream error flagged; set error message
        setErrorMessage("Stream error writing amp output (filename=\"" +
                                                           ampFName + "\"");
      }
    }
    catch(Exception ex)
    {    //exception error occurred; set error message
      setErrorMessage("Error writing to output:  " + ex);
    }
    return false;       //return error flag
  }
  ...
  public boolean writeAmpPhaseData(Writer ampOutStm,Writer phaseOutStm,
                                                           String headerStr)
  {
    return writeAmpPhaseData(ampOutStm,phaseOutStm,"(ampStream)",
                                                 "(phaseStream)",headerStr);
  }

    /**
     * Calculates and writes amplitude/phase response values generated
     * by 'calculateResponse()' to the two files with the given names.
     * Each outputted line contains 2 numbers:  the frequency followed by
     * the amplitude or phase response value for the frequency.  Each
     * number is written in floating-point exponent format with 7
     * significant digits.
     * @param outputDirectory output directory or null for current directory.
     * @param ampFName name of output file to write amplitude data to.
     * @param phaseFName name of output file to write phase data to.
     * @param headerStr a String of header information to be included in
     * the output, or null to indicate no header information.
     * @return true if successful, false if error (in which case an error
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeAmpPhaseData(
      File outputDirectory,String ampFName,String phaseFName,String headerStr)
  {
    final FileWriter ampOut;
    try
    {         //open file for output:
      ampOut = openFileWriter(outputDirectory,ampFName);
    }
    catch(Exception ex)
    {         //error opening file; set error message
      setErrorMessage("Error opening amp file (\"" + ampFName +
                                                  "\") for output:  " + ex);
      return false;
    }
    final FileWriter phaseOut;
    try
    {         //open file for output:
      phaseOut = openFileWriter(outputDirectory,phaseFName);
    }
    catch(Exception ex)
    {         //error opening file; set error message
      setErrorMessage("Error opening phase file (\"" + phaseFName +
                                                  "\") for output:  " + ex);
      return false;
    }
    final boolean retFlag = writeAmpPhaseData(ampOut,phaseOut,
                                             ampFName,phaseFName,headerStr);
    try
    {
      ampOut.close();             //close output files
      phaseOut.close();
    }
    catch(IOException ex) {}
    return retFlag;
    ...
    /**
     * Calculates and writes amplitude/phase response values generated
     * by 'calculateResponse()' to the given output stream.
     * Each outputted line contains 3 numbers:  the frequency followed by
     * the amplitude and phase response value for the frequency.  Each
     * number is written in floating-point exponent format with 7
     * significant digits.
     * @param outStm output stream to write to.
     * @param fName file name associated with output stream.
     ...
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeAmpPhaseData(Writer outStm,String fName,
                                                           String headerStr)
  {
    final AmpPhaseBlk [] ampPhaseArr;
    if((ampPhaseArr=getAmpPhaseArray()) == null) //calc & fetch A/P data
    ...
    try
    {
      final double [] freqArr = getCalcFreqArray();
      final int len = freqArr.length;
      PrintWriter out = new PrintWriter(new BufferedWriter(outStm));
      if(headerStr != null)
      {  //header string was provided
        out.println(headerStr);                  //show provided header info
        out.println(getDescHeaderString());      //add frequency info
        out.println(Run.HDR_CMT_STR.trim());     //add blank line
                                                 //add column headers:
        out.println(Run.HDR_CMT_STR + "  Freq      Amplitude      Phase");
      }
      for(int i=0; i<len; ++i)
      {  //for each entry in arrays; write line of output
        out.println(RespUtils.fmtNumber(freqArr[i]) + " " +
    ...
    /**
     * Calculates and writes amplitude/phase response values generated by
     * 'calculateResponse()' to the given output stream.
     * Each outputted line contains 3 numbers:  the frequency followed by
     * the amplitude and phase response value for the frequency.  Each
     * number is written in floating-point exponent format with 7
     ...
     * message may be fetched via the 'getErrorMessage()' method).
     */
  public boolean writeAmpPhaseData(Writer outStm,String headerStr)
  {
    return writeAmpPhaseData(outStm,"(stream)",headerStr);
  }
      ...
      File outputDirectory,String fName,String headerStr)
  {
    ...
    final boolean retFlag = writeAmpPhaseData(out,fName,headerStr);
    try { out.close(); }          //close output file
    catch(IOException ex) {}
    return retFlag;
  }
    ...
    /**
     * Returns a header information string describing the frequencies
     * used by this 'OutputGenerator', the output units convertion type,
     * the start and stop stages used (if not zero for all stages), and
     * the current date/time.
     * @return A new header information string.
     */
  protected String getDescHeaderString()
  {
    return getOutputHeaderString(Run.HDR_CMT_STR,", ",UtilFns.newline) +
                         UtilFns.newline + Run.HDR_CMT_STR + "Created by " +
                     Run.REVISION_STR + " on " + UtilFns.timeMillisToString(
     ...
     * Returns a header information string describing the frequencies
     * used by this 'OutputGenerator', the output units convertion type,
     * and the start and stop stages used (if not zero for all stages).
     ...
     * @return A new header information string.
     */
  public String getOutputHeaderString(String sLineStr,String sepStr,
                                                          String newlineStr)
  {
    ...
    if((freqArr=getCalcFreqArray()) == null ||
                                             (arrayLen=freqArr.length) <= 0)
    {    //no data; just put in start-line string
      retStr = sLineStr;
    }
    else
    {
      retStr = sLineStr + "MinFreq: " +
           decValFormatObj.format(freqArr[0]) + sepStr + "MaxFreq: " +
                         decValFormatObj.format(freqArr[arrayLen-1]) +
                                          sepStr + "NumFreqs: " + arrayLen +
            sepStr + "Spacing: " + (logSpacingFlag?"Logarithmic":"Linear") +
                                   newlineStr + sLineStr;
                   //add first item to second-line list
      secLineList.add("OutputUnits: " + getLongUnitConvString(outUnitsConv));
    }
    if(startStageNum > 0 || stopStageNum > 0)
    {    //non-zero start-stage number; show range of stages used
      secLineList.add("StartStage: " + startStageNum + sepStr +
                                              "StopStage: " + stopStageNum);
    }
    if(useEstDelayFlag)
      secLineList.add("UseEstDelay: true");
    if(listInterpOutFlag)
      secLineList.add("ListInterpOut: true");
    if(listInterpInFlag)
      secLineList.add("ListInterpIn: true");
    if(listInterpOutFlag || listInterpInFlag)
      secLineList.add("Tension: " + listInterpTension);
    if(unwrapPhaseFlag)
      secLineList.add("UnwrapPhase: true");
    if(totalSensitFlag)
      secLineList.add("TotalSensit: true");
    if(b62XValue > 0.0)
      secLineList.add("B62_x: " + b62XValue);

         //show A0 normalization from first stage (if available):
    final Stage stageObj;
    final Normalization normObj;
    if(respObj != null)
    {  //current response object is OK
      if(respObj.the_sensitivity != null)
      {  //reponse-sensitivity object found; display frequency
        secLineList.add("SensFreq=" +
                                  respObj.the_sensitivity.frequency + "Hz");
      }
      if(respObj.stages != null && respObj.stages.length > 0 &&
                                     (stageObj=respObj.stages[0]) != null &&
                                       stageObj.the_normalization != null &&
                                    stageObj.the_normalization.length > 0 &&
                            (normObj=stageObj.the_normalization[0]) != null)
      {  //first stage and normalization found; build display string
        secLineList.add("Norm" + A0_STR + '=' +
                                           normObj.ao_normalization_factor);
      }
    }
         //build second-line string one item at a time, checking length
         // before each item and inserting newline if necessary:
    final StringBuffer buff = new StringBuffer();
    final Iterator iterObj = secLineList.iterator();
    String str;
    final int sepLen = sepStr.length();
    int len, lineLen = 0;
    while(iterObj.hasNext())
    {  //for each second-line item
      str = (String)(iterObj.next());
      if(lineLen + sepLen + (len=str.length()) > 75)
      {  //item would make line too long
        buff.append(newlineStr);       //add newline
        buff.append(sLineStr);         //add start-line string
        lineLen = 0;                   //reinitialize line-length tracker
      }
      else if(lineLen > 0)
      {  //item will not make line too long and not first item on line
        buff.append(sepStr);           //add item separator
        lineLen += sepLen;             //add to line length
      }
      buff.append(str);           //add item
      lineLen += len;             //add to line length
    }
    return retStr + buff.toString();
  }
     ...
     * @return A new header information string.
     */
  public String getOutputHeaderString(String sepStr,String newlineStr)
  {
    return getOutputHeaderString("",sepStr,newlineStr);
  }

    /**
     * Returns the response-sensitivity factor.
     * @return A new Float object containing the response-sensitivity
     * factor, or null if none available.
     */
  public Float getRespSensitFactor()
  {
     ...
     * @return A new Float object containing the response-sensitivity
     * frequency, or null if none available.
     */
  public Float getRespSensitFreq()
  {
     ...
     * @return A new Float object containing the normalization factor for
  ...
  public Float getRespS1NormFactor()
  {
    final Stage stageObj;
    final Normalization normObj;
    return (respObj != null && respObj.stages != null &&
  ...
  public Float getRespS1NormFreq()
  {
    ...
    /**
     * @return the sensitivity value calculated by 'normalizeResponse()',
     * or 0.0 if 'normalizeResponse()' has not been performed.
     */
  public double getCalcSensitivity()
  {
    ...
    /**
     * @return the frequency used by 'normalizeResponse()',
     * or 0.0 if 'normalizeResponse()' has not been performed.
     */
  public double getCalcSenseFrequency()
  {
    ...
    /**
     * @return the first input unit processed by 'normalizeResponse()'
     * or 'calculateResponse()', or null if neither function has been
     * called.
     */
  public Unit getFirstUnitProc()
  {
    ...
    /**
     * @return the last output unit processed by 'normalizeResponse()'
     * or 'calculateResponse()', or null if neither function has been
     * called.
     */
  public Unit getLastUnitProc()
  {
     ...
     * 'calculateResponse()', or 0 if 'calculateResponse()'
     * has not been called.
     */
  public int getNumCalcStages()
  {
     ...
     * @return true if phase output values are to be unwrapped;
     * false if not.
     */
  public boolean getUnwrapPhaseFlag()
  {
    ...
    /**
     * Determines if stage 0 (total) sensitivity was used to calculate
     * response.
  ...
  public boolean getTotalSensitFlag()
  {
    ...
    /**
     * Returns true if the response contains a List (blockette 55) stage
     * and 'calculateResponse()' has been called.
     * @return true if the response contains a List (blockette 55) stage
     * and 'calculateResponse()' has been called.
     */
  ...
  /**
   * Prints messages about the current response's List stage to the
   * given print stream.
   * @param outStm the 'PrintStream' object to use.
   * @param ampPhaOutFlag true if amp/phase output is being generated;
   * false if complex/spectra output is being generated.
   */
  public void printListStageMsgs(PrintStream outStm, boolean ampPhaOutFlag)
  {
    final double [] dArr;   //get number of frequencies:
    final int nFreqs = ((dArr=getCalcFreqArray()) != null) ? dArr.length : 0;
                            //build descriptor string for freq count:
    final String fDescStr = ((nFreqs > 0) ? Integer.toString(nFreqs) :
                    "set of") + " frequenc" + ((nFreqs != 1) ? "ies" : "s");
    if(getListInterpInFlag())
    {   //using List-blockette "input" interpolation
      outStm.println("Note:  The input has been interpolated from " +
                                                 "the response List stage");
      outStm.println("(blockette 55) to generate output for the " +
                                                   fDescStr + " requested");
    }
    else if(getListInterpOutFlag() && ampPhaOutFlag)
    {   //using List-blockette "output" interpolation and amp/phsae output
      if(isAmpPhaseArrayCalculated())
      {       //amp/phase array has been calculated (and interpolated)
        outStm.println("Note:  The response List stage (blockette 55) " +
                                                         "output has been");
        outStm.println("interpolated to correspond to the " +
                                                   fDescStr + " requested");
      }
      else
      {       //amp/phase array has not been calculated
        outStm.println("Note:  The output has been interpolated from the " +
                                                                  fDescStr);
        outStm.println("defined in the response List stage" +
                                                         " (blockette 55)");
      }
    }
    else
    {   //not using interpolation
      outStm.println("WARNING:  Response contains a " +
                               "List stage (blockette 55)--the output has");
      outStm.println("been generated for the " +
                                    fDescStr + " defined in the blockette");
    }
  }
  ...
  {
    final double [] dArr;
    return (respArraysInfoArray != null && respArraysInfoArray[0] != null &&
                       (dArr=respArraysInfoArray[0].frequencyArr) != null) ?
                                                  dArr : requestedFreqArray;
  }

    /**
     * Returns a string of information about the response held by this
     * object.
     * @return a string of information about the response held by this
     * object.
     */
  public String getRespInfoString()
  {
    return "    SEED units:  In=" +
                                unitInfoStr(getFirstUnitProc()) + ", Out=" +
       unitInfoStr(getLastUnitProc()) + UtilFns.newline + "    Calc_sens=" +
                 RespUtils.fmtNumber(getCalcSensitivity()) + " (reported=" +
                RespUtils.fmtNumber(RespUtils.getRespSensitivity(respObj)) +
                     ") @ " + RespUtils.fmtNumber(getCalcSenseFrequency()) +
                               " Hz" + UtilFns.newline + "    Calc_delay=" +
       RespUtils.fmtNumber(getCalculatedDelay()) + ", Correction_applied=" +
             RespUtils.fmtNumber(getCorrectionApplied()) + UtilFns.newline +
               "    Est_delay=" + RespUtils.fmtNumber(getEstimatedDelay()) +
                                                ", Final_sample_interval=" +
                  RespUtils.fmtNumber(getSampleInterval()) + "(sec/sample)";
  }
     ...
     */
  public static String unitInfoStr(Unit unitObj)
  {
  ...
  protected void setInfoMessage(String str)
  {
    ...
    /**
     * Determines an info message has been entered.  The info message
     * may be fetched via the 'getInfoMessage()' method.
  ...
  public void clearInfoMessage()
  {
    ...
    /**
     * Returns a String containing a listing of the stages in the response,
     * in the format used by 'evalresp'.  A line is output for each stage
     * containing the stage sequence number, filter-type-name, normalization
     * value, and further information.
     * @return A String containing a listing of the stages in the response,
     * in the format used by 'evalresp'.
     */
  public String getStagesListStr()
  {
    final int tNLen = 10;    //field length for filter type name
                             //get number of stages in response:
    final int numStages = (respObj != null && respObj.stages != null) ?
                                                  respObj.stages.length : 0;
                   //setup index for first stage (zero-based):
    final int firstStageIdx = (startStageNum > 0) ? startStageNum-1 : 0;
                   //setup index for first stage; set last stage if stop<=0:
    final int lastStageIdx =
                           (stopStageNum > 0 && stopStageNum <= numStages) ?
                                               stopStageNum-1 : numStages-1;
    if(numStages <= 0 || firstStageIdx > lastStageIdx)
    {    //no response, no stages in response or none selected
      return "No stages";
    }
    final StringBuffer buff = new StringBuffer();
    Stage stageObj;
    Filter filterObj;
    CoefficientFilter coeffFilterObj;
    PoleZeroFilter poleZeroFilterObj;
    ListFilter listFilterObj;
    PolynomialFilter polyFilterObj;
    Decimation deciObj;
    TypeNormStringBlock typeNormStrBlkObj;
    Double doubleObj;
    int numPoles,numZeros,numNumers,numDenoms,numFreqs;
    int stageIdx = firstStageIdx;      //set index for start stage
    while(true)
    {    //for each stage in response
      if((stageObj=respObj.stages[stageIdx]) != null)
      {  //stage object OK; show stage number
        buff.append("    Stg" +      //pad stage # to 3 characters
           UtilFns.fixStringLen(Integer.toString(stageIdx+1),3,true,false) +
                                                                     ":  ");
        if(stageObj.filters != null && stageObj.filters.length > 0)
        {     //stage contains filters (not gain-only stage)
          if((filterObj=stageObj.filters[0]) != null)
          {   //filter object not null
            if(filterObj.discriminator().equals(FilterType.POLEZERO))
            {   //poles/zeros type filter
              if((poleZeroFilterObj=filterObj.pole_zero_filter()) != null)
              {      //pole/zero filter object fetched OK
                numPoles = (poleZeroFilterObj.poles != null) ?
                                         poleZeroFilterObj.poles.length : 0;
                numZeros = (poleZeroFilterObj.zeros != null) ?
                                         poleZeroFilterObj.zeros.length : 0;
              }
              else   //pole/zero filter object not OK
                numPoles = numZeros = 0;
                     //fetch type and normalization ID strings:
              typeNormStrBlkObj = getPolesZerosStageTypeStr(stageObj);
                     //if no stage type then show "(????)":
              final String typeStr = (typeNormStrBlkObj.typeStr != null &&
                                   typeNormStrBlkObj.typeStr.length() > 0) ?
                                    typeNormStrBlkObj.typeStr : NO_TYPE_STR;
                     //add stage type name string and norm field name:
              buff.append(UtilFns.fixStringLen(typeStr,tNLen,false,false) +
                                     " " + typeNormStrBlkObj.normStr + "=");
              if(stageObj.the_normalization != null &&
                                   stageObj.the_normalization.length > 0 &&
                                      stageObj.the_normalization[0] != null)
              {      //normalization object OK; get normalization factor
                buff.append(RespUtils.fmtNumber(
                    stageObj.the_normalization[0].ao_normalization_factor));
              }
              else   //no normalization object
                buff.append("1.0         ");
              buff.append(" " +     //add number of poles and zeros info
               UtilFns.fixStringLen(Integer.toString(numPoles),4,true,false)
                                     + " Pole" + ((numPoles!=1)?"s ":"  ") +
               UtilFns.fixStringLen(Integer.toString(numZeros),4,true,false)
                            + " Zero" + ((numZeros!=1)?"s ":"  ") + " Sd=");
              if(stageObj.the_gain != null)
              {      //stage gain object OK
                buff.append(        //add stage gain factor value
                        RespUtils.fmtNumber(stageObj.the_gain.gain_factor));
              }
              else
                buff.append("????");
            }
            ...
            {   //coefficients type filter
              if((coeffFilterObj=filterObj.coeff_filter()) != null)
              {      //coefficient filter object fetched OK
                numNumers = (coeffFilterObj.numerator != null) ?
                                        coeffFilterObj.numerator.length : 0;
                numDenoms = (coeffFilterObj.denominator != null) ?
                                      coeffFilterObj.denominator.length : 0;
              }
              else   //coefficient filter object not OK
                numNumers = numDenoms = 0;
                     //fetch type and normalization ID strings:
              typeNormStrBlkObj =
                            getCoefficientsStageTypeStr(stageObj,numDenoms);
                     //if no stage type then show "(????)":
              final String typeStr = (typeNormStrBlkObj.typeStr != null &&
                                   typeNormStrBlkObj.typeStr.length() > 0) ?
                                    typeNormStrBlkObj.typeStr : NO_TYPE_STR;
              ...
              {      //normalization object OK; get normalization factor
                ...
                buff.append("1.0         ");
              if(numDenoms > 0)
              {      //more then zero denominators; coefficients filter
                buff.append(" " +   //add number of numers and denoms
                                   UtilFns.fixStringLen(
                                   Integer.toString(numNumers),4,true,false)
                                   + " Numer" + ((numNumers!=1)?"s ":"  ") +
                                   UtilFns.fixStringLen(
                                   Integer.toString(numDenoms),3,true,false)
                                   + " Denom" + ((numDenoms!=1)?"s ":"  "));
              }
              else
              {      //no denominators; FIR filter
                buff.append(" " +   //add number of coefficients
                                   UtilFns.fixStringLen(
                                   Integer.toString(numNumers),4,true,false)
                                                          + " Coefficient" +
                                      ((numNumers!=1)?"s      ":"       "));
              }
              deciObj = null;     //initialize handle for decimation object
              if(stageObj.the_decimation != null &&
                                       stageObj.the_decimation.length > 0 &&
                             (deciObj=stageObj.the_decimation[0]) != null &&
                                     (doubleObj=RespUtils.deciToSampIntTime(
                                       stageObj.the_decimation[0])) != null)
              {      //decimation object and sample interval time fetched OK
                buff.append("SInt=" +
                              RespUtils.fmtNumber(doubleObj.doubleValue()));
              }
              else   //decimation obj & sample interval time not fetched OK
                buff.append("SInt=??????      ");
              if(stageObj.the_gain != null)
              {      //stage gain object OK
                buff.append(" Sd=" +      //add stage gain factor value
                        RespUtils.fmtNumber(stageObj.the_gain.gain_factor));
              }
              else
                buff.append(" Sd=????");
              if(deciObj != null)
              {  //decimation OK, check for negative corrAppl and estDelay
                if((doubleObj=RespUtils.quantityToIntTime(
                                     deciObj.correction_applied)) != null &&
                                              doubleObj.doubleValue() < 0.0)
                {  //correction-applied value is negative; append message
                  buff.append(UtilFns.newline + "Warning:  Stage " +
                           (stageIdx+1) + " negative correction_applied (" +
                              RespUtils.fmtNumber(doubleObj.doubleValue()) +
                                                      ") likely incorrect");
                }
                if((doubleObj=RespUtils.quantityToIntTime(
                                        deciObj.estimated_delay)) != null &&
                                              doubleObj.doubleValue() < 0.0)
                {  //estimated-delay value is negative; append message
                  buff.append(UtilFns.newline + "Warning:  Stage " +
                              (stageIdx+1) + " negative estimated_delay (" +
                              RespUtils.fmtNumber(doubleObj.doubleValue()) +
                                                      ") likely incorrect");
                }
              }
            ...
            {   //response list filter
              if((listFilterObj=filterObj.list_filter()) != null &&
                                            listFilterObj.frequency != null)
              {      //filter and freq array OK; get # of frequencies
                numFreqs = listFilterObj.frequency.length;
              }
              else
                numFreqs = 0;
                     //add stage type name string:
              buff.append(UtilFns.fixStringLen(LIST_STR,tNLen,false,false));
              if(numFreqs > 0)
              {      //array of frequencies contains elements
                buff.append(" minFreq=" + listFilterObj.frequency[0] +
                        "  maxFreq=" + listFilterObj.frequency[numFreqs-1] +
                                                  "  numFreqs=" + numFreqs);
              }
              else   //array of frequencies is empty
                buff.append(" (empty array of frequencies)");
            }
            else if(filterObj.discriminator().equals(FilterType.POLYNOMIAL))
            {   //polynomial type filter
              if((polyFilterObj=filterObj.polynomial_filter()) != null)
              {      //polynomial filter object fetched OK
                numNumers = (polyFilterObj.coeff_err_values != null) ?
                                  polyFilterObj.coeff_err_values.length : 0;
              }
              else   //polynomial filter object not OK
                numNumers = 0;
              buff.append(UtilFns.fixStringLen(POLYNOMIAL_STR,tNLen,
                                                              false,false));
              buff.append("    " + numNumers + " MacLaurin Coefficient" +
                                                   ((numNumers!=1)?"s":""));
            }
            else     //unknown filter type
              buff.append(UtilFns.fixStringLen("(????)",tNLen,false,false));
          }
          else  //filter object is null
            buff.append(UtilFns.fixStringLen("(null)",tNLen,false,false));
        }
        else
        {     //stage contains no filter objects
          if(stageObj.the_gain != null)
          {   //stage contains gain object (is a gain-only stage)
            buff.append(UtilFns.fixStringLen("GAIN",tNLen,false,false) +
               " Sd=" + RespUtils.fmtNumber(stageObj.the_gain.gain_factor));
          }
          else     //stage does not contain object
            buff.append(UtilFns.fixStringLen("(????)",tNLen,false,false));
        }
        if(++stageIdx > lastStageIdx)
          break;        //if no more stages then exit loop
        buff.append(UtilFns.newline);       //add newline separator
      }
      else if(++stageIdx > lastStageIdx)
        break;          //if no more stages then exit loop
    }
    return buff.toString();        //reutrn string version of buffer
  }

    /**
     * Returns a type identification string for the given stage.
     * @param stageObj stage object to use.
    ...
    {  //stage object OK
      ...
      {     //stage contains filters (not gain-only stage)
        ...
        {   //filter object not null
          ...
          if(filterObj.discriminator().equals(FilterType.COEFFICIENT))
          {   //coefficients type filter
            CoefficientFilter coeffFilterObj;
            int numDenoms;
            if((coeffFilterObj=filterObj.coeff_filter()) != null)
   ...
   * stage object.
   * A normalization identification string is also returned.
   * @param stageObj stage object to use.
  ...
  protected static TypeNormStringBlock getPolesZerosStageTypeStr(Stage stageObj)
  {
    ...
    if(stageObj.type != null)
    {      //stage type object OK; set string for type
  ...
  /**
   * Returns a type identification string for the given "coefficients"
   * stage object.
   * A normalization identification string is also returned.
   * @param stageObj stage object to use.
   * @param numDenoms number of denominators in filter for stage.
   * @return A new 'TypeNormStringBlock' object containing the type and
   * normalization identification strings.
   */
  protected static TypeNormStringBlock getCoefficientsStageTypeStr(
                                               Stage stageObj,int numDenoms)
  {
    String typeStr;
    String normStr = H0_STR;      //setup initial normalization value
    if(stageObj.type != null)
    {      //stage type object OK; set string for type
      ...
      {
        typeStr = (numDenoms > 0) ? "IIR_COEFFS" :
                                              determineFirTypeStr(stageObj);
      }
      else if(stageObj.type.equals(TransferType.LAPLACE) ||
                                  stageObj.type.equals(TransferType.ANALOG))
      {
        typeStr = ANALOG_STR;
        normStr = A0_STR;   //analog stage; change norm field name
      }
  ...
  protected static int determineFirTypeVal(Filter filterObj)
  {
    ...
    try
    {
      ...
      if(filterObj != null &&
                        (coeffFilterObj=filterObj.coeff_filter()) != null &&
                         (numeratorsArr=coeffFilterObj.numerator) != null &&
                                   (numNumerators=numeratorsArr.length) > 1)
      {  //numerators array OK and at least 2 entries
        int i = 0, j = numNumerators - 1;
        CoefficientErrored coeff1Obj,coeff2Obj;
        do
        {  //for each potential pair of symmetrical entries; check values
          if((coeff1Obj=numeratorsArr[i]) != (coeff2Obj=numeratorsArr[j]) &&
                                      (coeff1Obj.value != coeff2Obj.value ||
                                        coeff1Obj.error != coeff2Obj.error))
          {  //symmetry pairs do not match
    ...
    {  //some kind of exception error; just return 'FIR_UNKNOWN'
      ex.printStackTrace();
    }
    return FIR_UNKNOWN;
  }
  ...
  /**
   * Determines the symmetry type for the given stage object containing
   * an FIR-filter object.
   * @param stageObj stage object containing filter object.
  ...
  protected static String determineFirTypeStr(Stage stageObj)
  {
    ...
    final Filter filterObj = (stageObj.filters != null &&
                  stageObj.filters.length > 0) ? stageObj.filters[0] : null;
  ...
  /**
   * Returns a modified version of the given 'srcArr' array with any of
   * its entries outside of the given 'chkArr' array clipped.  If any
   * entries are clipped and 'showNotesFlag'==true then a message is
   * sent to 'stderr'.
   * @param srcArr source array.
   * @param chkArr check array.
   * @param showNotesFlag true to send "Note:" messages to 'stderr'.
   * @return A modified version of the given 'srcArr' array, or the
   * original array if no entries were clipped.
   */
  protected static double [] clipFreqArray(double [] srcArr, double [] chkArr,
                                                      boolean showNotesFlag)
  {
    final int srcArrLen;
    if(chkArr.length > 0 && (srcArrLen=srcArr.length) > 0)
    {    //arrays contain data
      double firstVal = chkArr[0];     //get first and last "check" values
      double lastVal = chkArr[chkArr.length-1];
      if(firstVal > lastVal)
      {  //first "check" value larger than last; swap them
        final double tmpVal = firstVal;
        firstVal = lastVal;
        lastVal = tmpVal;
      }
      int sPos = 0;
      while(sPos < srcArrLen &&
                        (srcArr[sPos] < firstVal || srcArr[sPos] > lastVal))
      {  //for each out-of-range entry at beginning of "source" array
        ++sPos;
      }
      ...
      while(ePos > 0 && (srcArr[ePos] > lastVal || srcArr[ePos] < firstVal))
      {  //for each out-of-range entry at end of "source" array
        --ePos;
      }
      ...
      if(ePos < srcArrLen-1 &&
                            Math.abs(srcArr[ePos+1]-lastVal) < lastVal*1e-6)
      {
      ...
      final int retArrLen;
      if((retArrLen=ePos-sPos+1) < srcArrLen || fixFirstFlag || fixLastFlag)
      {  //at least one entry was clipped or first or last value "fixed"
        ...
        if(showNotesFlag)
        {     //"Note:" messages to 'stderr' enabled
          if(sPos > 0)
          {     //at least one entry clipped from beginning; show note
            System.err.println("Note:  " + sPos + " frequenc" +
                                               ((sPos != 1) ? "ies" : "y") +
                              " clipped from beginning of requested range");
          }
          if((ePos=srcArrLen-ePos-1) > 0)
          {     //at least one entry clipped from beginning; show note
            System.err.println("Note:  " + ePos + " frequenc" +
                                               ((ePos != 1) ? "ies" : "y") +
  ...
  /**
   * Unwraps the given array of 'phase' values.  A phase array is "wrapped"
   * by adding +/-360 to portions of the dataset to make all the array
   * values be between -180 and +180 (inclusive).  This method "unwraps"
   * the given array by detecting transitions where the dataset has been
   * "wrapped" and adding +/-360 to restore the "original" values.
   * @param srcPhaseArr phase array to unwrap.
   * @param firstNonNegFlag true if first phase value should always be
   * made non-negative; false if not.
   * @return A new 'double' array containing the unwrapped values, or
   * the given array if it was not wrapped.
   */
                                                    ...
                                                    boolean firstNonNegFlag)
  {
    ...
    if(firstNonNegFlag && prevPhaseVal < 0.0)
    {  //flag set and first value is negative
    ...
    for(int i=1; i<srcPhaseArrLen; ++i)
    {    //for each remaining value in source array
      newPhaseVal = srcPhaseArr[i] + offsetVal;
      if((diff=newPhaseVal-prevPhaseVal) > 180.0)
      {  //phase "wrap" transition detected
  ...
  /**
   * Wraps the given array of 'phase' values.  A phase array is "wrapped"
   * by adding +/-360 to portions of the dataset to make all the array
   * values be between -180 and +180 (inclusive).
   * @param srcPhaseArr phase array to wrap.
   * @return A new 'double' array containing the wrapped values, or
   * the given array if it did not need to be wrapped.
   */
  protected static double [] wrapPhaseArray(double [] srcPhaseArr)
  {
    ...
    final double [] retArr = new double[srcPhaseArrLen];
    for(int i=0; i<srcPhaseArrLen; ++i)
    {    //for each value in source array
      newPhaseVal = srcPhaseArr[i] + offsetVal;
      if(newPhaseVal > 180.0)
      {  //phase value too high
        offsetVal -= 360.0;            //adjust offset
        newPhaseVal -= 360.0;          //adjust phase value
  ...
  /**
   * Returns an array containing the amplitude values held by the given
   * array of 'AmpPhaseBlk' objects.
   * @return A new 'double' array containing the amplitude values held by
   * the given array of 'AmpPhaseBlk' objects, or an empty array if none
   * are available.
   * @param ampPhaseArr array of 'AmpPhaseBlk' objects.
   */
  ...
  /**
   * Returns an array containing the phase values held by the given
   * array of 'AmpPhaseBlk' objects.
  ...
  public static double [] fetchAmpPhaPhaseArray(AmpPhaseBlk [] ampPhaseArr)
  {
     ...
     * @param freq the frequency value to use.  If a Laplace filter then
     * the frequency should be multplied by 2*pi.
     * @return A 'ComplexBlk' object containing the response.
     */
  public static ComplexBlk analogTrans(PoleZeroFilter filterObj,
                                                double normFact,double freq)
  {
//    if(XDEBUG_FLAG)
//    {    //send debug message to default log file
...
//    }
    ...
    final ComplexBlk denom = new ComplexBlk(1.0,1.0);
                        //get number of zeros:
    final int numZeros = (filterObj != null && filterObj.zeros != null) ?
                                                 filterObj.zeros.length : 0;
                        //get number of poles:
    final int numPoles = (filterObj != null && filterObj.poles != null) ?
                                                 filterObj.poles.length : 0;
    int i;
    for(i=0; i<numZeros; i++)
    {    //for each zero, numerator=numerator*(omega-zero[i])
//      temp.real = omega.real - filterObj.zeros[i].real;
//      temp.imag = omega.imag - filterObj.zeros[i].imaginary;
//      zMultiply(num,temp);
      num.zMultiply(omega.real-filterObj.zeros[i].real,
                                   omega.imag-filterObj.zeros[i].imaginary);
    }
    for(i=0; i<numPoles; i++)
    {    //for each pole, denominator=denominator*(omega-pole[i])
//      temp.real = omega.real - filterObj.poles[i].real;
//      temp.imag = omega.imag - filterObj.poles[i].imaginary;
//      zMultiply(denom,temp);
      denom.zMultiply(omega.real-filterObj.poles[i].real,
                                   omega.imag-filterObj.poles[i].imaginary);
    }
         //gain*num/denum
...
//    }
     ...
     * @return A 'ComplexBlk' object containing the response.
     */
  public static ComplexBlk iirPzTrans(PoleZeroFilter filterObj,
                           double normFact,double sIntervalTime,double wVal)
  {
//    if(XDEBUG_FLAG)
//    {    //send debug message to default log file
...
//    }
    return new ComplexBlk(mod*Math.cos(pha)*normFact,
                                                mod*Math.sin(pha)*normFact);
  }
     ...
     * @return A 'ComplexBlk' object containing the response.
     */
  public static ComplexBlk iirTrans(CoefficientFilter filterObj,
                           double normFact,double sIntervalTime,double wVal)
  {
//    if(XDEBUG_FLAG)
//    {    //send debug message to default log file
...
//    }
    ...
    final int numNumers = (filterObj != null &&
              filterObj.numerator != null) ? filterObj.numerator.length : 0;
                        //get number of denominators:
    final int numDenoms = (filterObj != null &&
          filterObj.denominator != null) ? filterObj.denominator.length : 0;
                        //calculate radial freq. time sample interval:
    final double wsint = wVal * sIntervalTime;
...
//    }
    return new ComplexBlk(amp*Math.cos(phase)*normFact,
                                              amp*Math.sin(phase)*normFact);
 }

    /**
     * Calculates the response of a digital FIR filter.  Only the
     * numerators of the given filter object are used.
     * @param filterObj a coefficients filter object.
     * @param normFact the normalization factor to use.
     * @param sIntervalTime the sample interval time to use.
     * @param wVal the frequency value to use.
     ...
     * @return A 'ComplexBlk' object containing the response.
     */
  public static ComplexBlk firTrans(CoefficientFilter filterObj,
          double normFact,double sIntervalTime,double wVal,int firTypeVal)
  {
//    if(XDEBUG_FLAG)
//    {    //send debug message to default log file
...
//    }
    ...
    {  //FIR type is symmetrical 1
      ...
      return new ComplexBlk(
                    (filterObj.numerator[i].value+(2.0*rVal))*normFact,0.0);
    }
    else if(firTypeVal == FIR_SYM2)
    {  //FIR type is symmetrical 2
      ...
      {
        factVal = numNumerators - (i+1);
        rVal += filterObj.numerator[i].value *
                                      Math.cos(wsint*((double)factVal+0.5));
      }
      return new ComplexBlk(2.0*rVal*normFact,0.0);
    }
    else
    {  //FIR type is asymmetrical
      ...
      do
      {
        ...
        {  //all coefficients checked
          return new ComplexBlk(((wsint == 0.0) ? 1.0 :
...
//      double pha = Math.atan2(iVal,rVal);
      ...
      double pha = Math.atan2(iVal,rVal) +
                           (wVal*(double)((numCoeffs-1)/2.0)*sIntervalTime);
...
//      }
    ...
    /**
     * Checks if the FIR coefficients filter should be normalized to 1.0
     * at frequency zero and adjusts the filter values if so.
     * @param filterObj a coefficients filter object.
  ...
  public void checkFixFirFreq0Norm(CoefficientFilter filterObj, int stageNum)
  {
    ...
    final int numCoeffs = (filterObj != null &&
              filterObj.numerator != null) ? filterObj.numerator.length : 0;
    if(numCoeffs > 0)
    {    //more than zero coefficients (numerators)
      double sumVal = 0.0;             //calculate sum of coefficients:
      for(int i=0; i<numCoeffs; ++i)
        sumVal += filterObj.numerator[i].value;
      if(sumVal < (1.0-FIR_NORM_TOL) || sumVal > (1.0+FIR_NORM_TOL))
      {  //sum of coefficients is not 1.0
                   //divide by sum to make sum of coefficients be 1.0:
        for(int i=0; i<numCoeffs; ++i)
          filterObj.numerator[i].value /= sumVal;
                   //set info message:
        setInfoMessage("WARNING:  FIR blockette normalized, sum[coef]=" +
                                 sumVal + " (stage #" + (stageNum+1) + ")");
      }
    }
  }

    /**
     * Converts the given unit object to a UNIT_CONV index
     * values or -1 if a conversion could not be made.
     * @param unitObj unit object.
     * @return One of the '..._UNIT_CONV' index
     * values or -1 if a conversion could not be made.
     */
  public static int toUnitConvIndex(Unit unitObj)
  {
    ...
    if((len=unitsArr.length) > 0 && (unitObj=unitsArr[0]) != null &&
                                            unitObj.the_unit_base != null &&
                             unitObj.the_unit_base.equals(UnitBase.METER) &&
                                                      unitObj.exponent == 1)
    {
      if(len <= 1)                       //if only 'meters' then
        return DISPLACE_UNIT_CONV;       //return 'displacement'
              //second unit must be seconds:
      if((unitObj=unitsArr[1]) != null && unitObj.the_unit_base != null &&
                              unitObj.the_unit_base.equals(UnitBase.SECOND))
      {
        if(unitObj.exponent == -1)
        {
          ...
          if(len == 3 && (unitObj=unitsArr[2]) != null &&
                                            unitObj.the_unit_base != null &&
                            unitObj.the_unit_base.equals(UnitBase.SECOND) &&
                                                     unitObj.exponent == -1)
          {
     ...
     * @param unitObj unit object.
     * @return One of the unit conversion strings.
     * ("def", "dis", "vel", "acc"), or an emptry string if a conversion
     * could not be made.
     */
  public static String toUnitConvString(Unit unitObj)
  {
    final int val;
    return ((val=toUnitConvIndex(unitObj)) >= 0 &&
                    val < UNIT_CONV_STRS.length) ? UNIT_CONV_STRS[val] : "";
  }

    /**
     * Converts the given output-units-conversion index to a string.
     * @param indexVal one of the ".._UNIT_CONV" index values.
     * @return One of the unit conversion strings ("Default",
     * "Displacement", "Velocity", "Acceleration"), or "???"
     * if a conversion could not be made.
     */
  public static String getLongUnitConvString(int indexVal)
  {
    return (indexVal >= 0 && indexVal < UNIT_CONV_LONGSTRS.length) ?
                                       UNIT_CONV_LONGSTRS[indexVal] : "???";
  }


  /**
   * Class TypeNormStringBlock defines a data block containing two
   * strings, one for "type" and one for "normalization".
   */
  public static class TypeNormStringBlock
  {
    ...
    /**
     * Creates a data block containing two strings, one for "type" and
     * one for "normalization".
     * @param typeStr type string.
     * @param normStr normalization string.
     */
