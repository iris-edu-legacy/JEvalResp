...
import static com.isti.jevalresp.x2r.ConvertingReader.toReader;
...
public class RespFileParser
{
  ...
  private Sensitivity curSensitivityObj = null;  //sensitivity for response
     ...
     * @param fNameStr the name of an input file from which to read data.
     */
  public RespFileParser(String fNameStr)
  {
    ...
    if(rdr == null)
    {    //unable to open input file (as a local file)
      try
      {       //attempt to open as a URL path:
        final String urlStr = RespUtils.fileObjPathToUrlStr(fNameStr);
        if((rdr=FileUtils.fileMultiOpen(urlStr)) != null)
        {     //opened successfully as a URL path
          fNameStr = urlStr;      //enter "restored" URL path
          clearErrorMessage();    //clear previous error message
        }
        else
        {     //unable to open as a URL path
          if(UtilFns.isURLAddress(urlStr))
          {   //name is a URL
            fNameStr = urlStr;         //enter "restored" URL path
      ...
      {  //some kind of exception error; enter new error message
        rdr = null;
        clearErrorMessage();
        setErrorMessage("Unable to open input file:  " + ex);
      }
    ...
    /**
     * Creates an 'rdseed' ASCII file parsing object.
     * @param in an input stream from which to read data.
     * @param fNameStr file name associated with the given input stream.
     */
  public RespFileParser(InputStream  in, String fNameStr)
  {
    inReaderObj = toReader(in);            //save input reader object
    inputFileName = fNameStr;     //enter file name
    if(inReaderObj != null)
    {    //stream object handle OK
      inTokens = new RespTokenizer(inReaderObj);    //create tokenizer
      checkInput();                         //check input data
    }
    else
    {    //null handle
      inTokens = null;       //set tokenizer to null; set error message
     ...
     * @param in an input stream from which to read data.
     */
  public RespFileParser(InputStream in)
  {
    this(in, "(stream)");
  }
  ...
  private void checkInput()
  {
    try
    {
      int tType;
      while(true)
      {       //for each token parsed
        if((tType=inTokens.nextToken()) == RespTokenizer.TT_EOF)
        {     //end-of-file reached; set error message
          setErrorMessage("No data found");
          break;
        }
        if(tType == RespTokenizer.TT_WORD)
        {     //word token found; check if station ID item
          if((firstBFSpec=parseBlockFieldNums(inTokens.getTokenString())) ==
                                         null || firstBFSpec.blockNum != 50)
          {   //station ID line not found; set error message
            setErrorMessage("No valid response data found at line " +
                                                         inTokens.lineno());
          }
          break;
        }
    ...
    {         //error reading token; set error message
  ...
  public void close()
  {
    ...
    /**
     * Finds a station/channel ID entry.  The name pattern parameters
     * are expected to be glob-style expressions (can have '*' or '?'
     * characters).  After a station/channel ID entry is found the
     * 'readResponse()' method may be used to parse and return the
     * response data.
     ...
     * @param sitePatArr an array of site name patterns to search for,
     ...
     * @return A new 'ChanIdHldr' object filled with data from the
     * station/channel ID found, or null if no matching ID was found
     * (in which case 'getErorFlag()' may be used to see if any errors
     * were detected and 'getErorMessage()' may be used to see information
     * about the first error detected).
     */
  public ChanIdHldr findChannelId(String [] stationPatArr,
                            String [] channelPatArr,String [] networkPatArr,
                     String [] sitePatArr,Date beginDateObj,Date endDateObj)
  {
    String staNameStr,chaNameStr,netNameStr,siteNameStr,parsedStartDateStr,
           parsedEndDateStr;
    Date parsedStartDateObj,parsedEndDateObj;
    if(inTokens == null)
    {    //tokenizer not set up; set error message
      setErrorMessage("Unable to read from input file");
      return null;
    }
    if(firstBFSpec == null || firstBFSpec.blockNum != 50 ||
                                                  firstBFSpec.fieldNum != 3)
    {    //station ID line was not found; set error message
      setErrorMessage("Valid response data not found at line " +
                                                         inTokens.lineno());
      return null;
    }
    ...
    while(true)
    {    //for each station/channel ID searched in file
      if((staNameStr=readBasicDataValue()) == null)   //fetch station name
      {  //error parsing station name; set error message
        setErrorMessage("Unable to parse station name at line " +
                                                         inTokens.lineno());
        return null;
      }
         //fetch network name:
      if((netNameStr=readBlockFieldValue(50,16)) == null)
      {  //error parsing network name; set error message
        setErrorMessage("Unable to parse network name at line " +
                                                         inTokens.lineno());
        return null;
      }
      if(netNameStr.equals("??"))      //if "??" network name then
        netNameStr = "";               //change to empty string
         //fetch location/site and channel names:
         // (locate/site name is optional)
      if((bfSpecObj=readNextBlockFieldNums(52,3,52,4)) != null)
      {       //next block/field numbers fetched OK
        if(bfSpecObj.blockNum == 52 && bfSpecObj.fieldNum == 3)
        {     //block and field numbers for location/site name found
//          if((siteNameStr=readBasicDataValue()) == null)   //fetch value
//          {        //error fetching value; set error message
//            setErrorMessage("Error parsing location/site name (sta=\"" +
//                           staNameStr + "\") at line " + inTokens.lineno());
//            return null;
//          }
          siteNameStr = readBasicDataValue();    //fetch site/location value
               //if blank or "??" site name then change to empty string:
          if(siteNameStr == null || siteNameStr.trim().length() <= 0 ||
                                                   siteNameStr.equals("??"))
          {
            siteNameStr = "";
          }
        ...
        siteNameStr = "";         //set empty string
      if(bfSpecObj == null || bfSpecObj.blockNum != 52 ||
                                                  bfSpecObj.fieldNum != 4 ||
                                  (chaNameStr=readBasicDataValue()) == null)
      {       //error fetching matching item or value; set error message
        setErrorMessage("Unable to parse channel name (sta=\"" +
                           staNameStr + "\") at line " + inTokens.lineno());
        return null;
      }
         //fetch begin date (all tokens to end of line):
      if((parsedStartDateStr=readBlockFieldValue(52,22,true)) == null ||
         (parsedStartDateObj=RespUtils.parseRespDate(parsedStartDateStr)) ==
                                                                       null)
      {  //error parsing begin date; set error message
        setErrorMessage("Unable to parse begin date (sta=\"" +
                                    staNameStr + "\", cha=\"" + chaNameStr +
                                        "\") at line " + inTokens.lineno());
        return null;
      }
         //fetch end date (all tokens to end of line):
      if((parsedEndDateStr=readBlockFieldValue(52,23,true)) == null)
      {       //field not found; set error message
        setErrorMessage("Unable to find end date field (sta=\"" +
                                    staNameStr + "\", cha=\"" + chaNameStr +
                                        "\") at line " + inTokens.lineno());
        return null;
      }
      if(!parsedEndDateStr.equalsIgnoreCase("No Ending Time"))
      {  //not special "No Ending Time" string; parse into Date object
        if((parsedEndDateObj=RespUtils.parseRespDate(parsedEndDateStr)) ==
                                                                       null)
        {     //error parsing end date; set error message
          setErrorMessage("Unable to parse end date (sta=\"" +
                                    staNameStr + "\", cha=\"" + chaNameStr +
                                        "\") at line " + inTokens.lineno());
          return null;
        }
        ...
        parsedEndDateObj = null;       //indicate no ending date

         //test if found channel ID is a match:
      if((stationPatArr == null || stationPatArr.length <= 0 ||
                  RespUtils.globStringArrMatch(staNameStr,stationPatArr)) &&
         (channelPatArr == null || channelPatArr.length <= 0 ||
                  RespUtils.globStringArrMatch(chaNameStr,channelPatArr)) &&
         (networkPatArr == null || networkPatArr.length <= 0 ||
                  RespUtils.globStringArrMatch(netNameStr,networkPatArr)) &&
         (sitePatArr == null || sitePatArr.length <= 0 ||
                  RespUtils.globStringSiteArrMatch(siteNameStr,sitePatArr)))
      {  //matching station/channel ID found; see if dates match
              //if no end date then treat 'beginDateObj' as a single date
              // to be matched by being >= parsed-start-date and <=
              // parsed-end-date; otherwise 'beginDateObj' needs to be
              // <= parsed-end-date and 'endDateObj' needs to be >=
              // parsed-start-date:
        if((endDateObj == null && (beginDateObj == null ||
                         (beginDateObj.compareTo(parsedStartDateObj) >= 0 &&
                                                (parsedEndDateObj == null ||
                        beginDateObj.compareTo(parsedEndDateObj) <= 0)))) ||
           (endDateObj != null &&
                            endDateObj.compareTo(parsedStartDateObj) >= 0 &&
                        (beginDateObj == null || parsedEndDateObj == null ||
                            beginDateObj.compareTo(parsedEndDateObj) <= 0)))
        {     //date matched OK; accept response
//          System.out.println("Response match in \"" + inputFileName +
//                         "\":  sta=" + staNameStr + ", cha=" + chaNameStr +
...
//            ", start=" + parsedStartDateStr + ", end=" + parsedEndDateStr);
      ...
      try
      {
        ...
        {                    //parse through any blank or comment lines:
          ...
          if(tType != RespTokenizer.TT_WORD || (bfSpecObj=
                    parseBlockFieldNums(inTokens.getTokenString())) == null)
          {   //not word token or not "B###F##" format string
            setErrorMessage("Invalid format in file at line " +
                                                         inTokens.lineno());
            return null;
          }
          if(bfSpecObj.blockNum == 50)
          {   //station ID data found
            if(bfSpecObj.fieldNum != 3)
            {      //not field number for station name; set error message
              setErrorMessage("Invalid format in file at line " +
                                                         inTokens.lineno());
     ...
     * @param networkPatArr an array of network name patterns to search for,
     * or a null or empty array to accept all network names.
     * @param sitePat a site name pattern to search for,
  ...
  public ChanIdHldr findChannelId(String [] stationPatArr,
                            String [] channelPatArr,String [] networkPatArr,
                           String sitePat,Date beginDateObj,Date endDateObj)
  {
    final String [] sitePatArr = (sitePat != null &&
                     sitePat.length() > 0) ? new String [] {sitePat} : null;
    return findChannelId(stationPatArr,channelPatArr,
                          networkPatArr,sitePatArr,beginDateObj,endDateObj);
  }
     ...
     * @param channelPatArr an array of channel name patterns to search for,
     * or a null or empty array to accept all channel names.
     * @param networkPat a network name pattern to search for,
     ...
     */
  public ChanIdHldr findChannelId(String [] stationPatArr,
                                  String [] channelPatArr,String networkPat,
                     String [] sitePatArr,Date beginDateObj,Date endDateObj)
  {
    final String [] networkPatArr = (networkPat != null &&
               networkPat.length() > 0) ? new String [] {networkPat} : null;
    return findChannelId(stationPatArr,channelPatArr,
                          networkPatArr,sitePatArr,beginDateObj,endDateObj);
  }
                           ...
                           String sitePat,Date beginDateObj,Date endDateObj)
  {
     ...
     * @param stationPatArr an array of station name patterns to search for,
     * or a null or empty array to accept all station names.
     * @param channelPat a channel name pattern to search for,
     ...
     */
  public ChanIdHldr findChannelId(String [] stationPatArr,
                                        String channelPat,String networkPat,
                           String sitePat,Date beginDateObj,Date endDateObj)
  {
    final String [] channelPatArr = (channelPat != null &&
               channelPat.length() > 0) ? new String [] {channelPat} : null;
    final String [] networkPatArr = (networkPat != null &&
               networkPat.length() > 0) ? new String [] {networkPat} : null;
    final String [] sitePatArr = (sitePat != null &&
                     sitePat.length() > 0) ? new String [] {sitePat} : null;
     ...
     * are expected to be glob-style expressions (can have '*' or '?'
     * characters).
     * @param stationPat a station name pattern to search for,
     ...
     * about the first error detected).
     */
  public ChanIdHldr findChannelId(String stationPat,String channelPat,
         String networkPat,String sitePat,Date beginDateObj,Date endDateObj)
  {
    final String [] stationPatArr = (stationPat != null &&
               stationPat.length() > 0) ? new String [] {stationPat} : null;
    final String [] channelPatArr = (channelPat != null &&
               channelPat.length() > 0) ? new String [] {channelPat} : null;
    final String [] networkPatArr = (networkPat != null &&
               networkPat.length() > 0) ? new String [] {networkPat} : null;
    ...
    /**
     * Parses and returns a set of response data.  The 'findChannelId()'
     * method should be used to find the desired station/channel ID before
     * this method is used.
     * @return A new 'Response' object filled with parsed data, or null if
     * an error occurred (in which case 'getErorMessage()' may be used to
  ...
  public Response readResponse()
  {
    ...
    if((bfSpecObj=readNextBlockFieldNums()) == null)
    {    //error fetching first "B###F##" item
      setErrorMessage("Unable to read any valid response data at line " +
                                                         inTokens.lineno());
      return null;
    }
    ...
    while(true)
    {
      switch(bfSpecObj.blockNum)
      {
        ...
        default:   //unexpected blockette; set error message
          setErrorMessage("Unexpected Blockette type (" +
                                                  bfSpecObj.blockNum + ")");
          return null;
      }
      ...
      if(tType != RespTokenizer.TT_WORD ||
         (bfSpecObj=parseBlockFieldNums(inTokens.getTokenString())) == null)
      {       //valid "B###F##..." string not found; set error message
        setErrorMessage("Invalid data in input file at line " +
                                                         inTokens.lineno());
        return null;
      }
    }
    ...
    try
    {
      stageArr = new Stage[curStagesVec.size()];
      stageArr = (Stage [])(curStagesVec.toArray(stageArr));
    }
    catch(Exception ex)
    ...
    {    //for each stage in response
      if((stageObj=stageArr[i]) == null)
      {  //array element is null; set error message
        setErrorMessage("No data entered for stage #" + (i+1) +
                                " (out of " + stageArr.length + " stages)");
        return null;
      }
      ...
      {  //no type object; check if stage is a gain-only stage
        if(stageObj.the_gain == null || stageObj.filters.length > 0)
        {     //no gain entered or more than 0 filters; set error message
        ...
        {     //find input units type of next non-gain-only stage:
          ...
          do
          {   //check each proceeding stage until valid input units found
            if(++j >= stageArr.length)
            {      //no more stages left; set error message
              setErrorMessage("Unable to find valid non-gain-only " +
                                                   "stage after #" + (i+1));
              return null;
            }
          }        //loop if next stage or its input units are invalid:
          while((nextStageObj=stageArr[j]) == null ||
                                         nextStageObj.input_units == null);
      ...
      {  //stage type OK (not gain-only stage or invalid type)
  ...
  private Stage createAddNewStageObj(int seqNum, boolean forceFlag)
  {
    ...
    {  //some kind of exception; just return null
    }
    return null;
  }
  ...
  private boolean readPolesZeros(BlockFieldSpec initBfSpecObj,
                                                           boolean dictFlag)
  {
    ...
    if(initBfSpecObj.blockNum != pzBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                     (typeStr=readBasicDataValue()) == null)
    {    //type field not found; set error message
      setErrorMessage("Unable to find response type field in Poles/Zeros" +
                                " Blockette at line " + inTokens.lineno());
      return false;
    }
    Stage stageObj = null;
    String str;
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if((str=readBlockFieldValue(pzBlkNum,++fieldNum)) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Poles/Zeros " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      try
      {            //convert stage seq# string to integer:
        seqNum = Integer.parseInt(str);
      }
      catch(NumberFormatException ex)
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                 ") in Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
    ...
    if(stageObj == null)
    {    //stage sequence number is not new; use latest stage
      final int len;
      ...
      stageObj = (Stage)obj;
//      if(stageObj.filters != null)
//      {       //stage already has a filter; set error message
//        setErrorMessage("Extra Poles/Zeros Blockette in same stage seq# (" +
//                         curStageSeqNum + ") at line " + inTokens.lineno());
//        return false;
//      }
    }
    ...
    {    //unrecognized value; set error message
      setErrorMessage("Invalid transfer type (\"" + typeStr +
               "\") in Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.type != null && !typeObj.equals(stageObj.type))
    {    //different stage type set previously
      setErrorMessage("Different transfer type (\"" + typeStr +
                     "\") in same stage in Poles/Zeros Blockette at line " +
                                                         inTokens.lineno());
      return false;
    }
    ...
    if((str=readBlockFieldValue(pzBlkNum,++fieldNum,true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in Poles/Zeros " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    {    //no previous in-units
      if((unitObj=RespUtils.respStrToUnit(str)) == null)
      {  //unrecognized in-units field value; set error message
        setErrorMessage("Invalid in-units type (\"" + str +
               "\") in Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
      stageObj.input_units = unitObj;       //enter new units value
    }
         //process out-units type value:
    if((str=readBlockFieldValue(pzBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in Poles/Zeros " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    {    //no previous out-units
      if((unitObj=RespUtils.respStrToUnit(str)) == null)
      {  //unrecognized out-units field value; set error message
        setErrorMessage("Invalid out-units type (\"" + str +
               "\") in Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
      stageObj.output_units = unitObj;      //enter new units value
    }
         //process normalization factor:
    if((str=readBlockFieldValue(pzBlkNum,++fieldNum)) == null)
    {    //normalization factor field not found; set error message
      setErrorMessage("Unable to find normalization factor field in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse normalization factor (" + str +
                 ") in Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
         //process normalization frequency:
    if((str=readBlockFieldValue(pzBlkNum,++fieldNum)) == null)
    {    //normalization frequency field not found; set error message
      setErrorMessage("Unable to find normalization frequency field in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse normalization frequency (" + str +
                 ") in Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.the_normalization == null)
    {    //normalization object not previously entered
              //create normalization object and enter into stage:
      stageObj.the_normalization = new Normalization[] {
                                     new Normalization(nFactVal,nFreqVal) };
    }
    else
    {    //normalization object was previously entered
      if(stageObj.the_normalization.length <= 0 ||
        nFactVal != stageObj.the_normalization[0].ao_normalization_factor ||
               nFreqVal != stageObj.the_normalization[0].normalization_freq)
      {       //previous normalization is different; set error message
        setErrorMessage("Different normalization values in same stage in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    if((str=readBlockFieldValue(pzBlkNum,++fieldNum)) == null)
    {    //number of zeros field not found; set error message
      setErrorMessage("Unable to find number of zeros field in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    final int numZeros;
    ...
    {
      setErrorMessage("Unable to parse number of zeros field (" + str +
                 ") in Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
         //process number of poles count (field number is +5 after zeros):
    if((str=readBlockFieldValue(pzBlkNum,fieldNum+5)) == null)
    {    //number of poles field not found; set error message
      setErrorMessage("Unable to find number of poles field in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    final int numPoles;
    ...
    {
      setErrorMessage("Unable to parse number of poles field (" + str +
                 ") in Poles/Zeros Blockette at line " + inTokens.lineno());
      return false;
    }
    BlockFieldSpec bfSpecObj;
    ...
    {    //for each line of poles items
              //read and check "B###F##" string; read and discard integer
              // index value; then read 4 floating-point values:
      if((bfSpecObj=readNextBlockFieldNums(pzBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != pzBlkNum || bfSpecObj.fieldNum != fieldNum ||
                                      bfSpecObj.endFieldNum != fieldNum+3 ||
                                            readNextTokenString() == null ||
                           (polesArr[i]=readComplexNumberErrored()) == null)
      {
        setErrorMessage("Error parsing poles data items in " +
                      "Poles/Zeros Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    {    //previous filter entered; append new values
      try
      {
        ...
        pzFilter.zeros = (ComplexNumberErrored [])
                        (vec.toArray(new ComplexNumberErrored[vec.size()]));
      }
      catch(Exception ex)
      {
        setErrorMessage("Internal error:  Exception extending arrays " +
                                           "in 'readPolesZeros()':  " + ex);
        return false;
      }
    }

/*
         //create and configure Poles/Zeros filter:
    final Filter filterObj = new Filter();
    filterObj.pole_zero_filter(new PoleZeroFilter(polesArr,zerosArr));
         //enter new filter into stage:
    if(stageObj.filters == null || stageObj.filters.length <= 0)
    {    //no previous filters entered
              //create and enter array containing filter item:
      stageObj.filters = new Filter[] { filterObj };
    }
    else
    {    //previous filter(s) entered
              //create new array with one more entry:
      final Filter [] filterArr = new Filter[stageObj.filters.length+1];
              //copy previous entries into new array:
      for(i=0; i<stageObj.filters.length; ++i)
        filterArr[i] = stageObj.filters[i];
      filterArr[i] = filterObj;        //copy in new entry
      stageObj.filters = filterArr;    //enter new array into stage
    }
*/
    return true;
  }
    ...
    // the response.
    // @param dictFlag true for dictionary blockette, false for not.
  private boolean readCoefficients(BlockFieldSpec initBfSpecObj,
                                                           boolean dictFlag)
  {
    ...
    if(initBfSpecObj.blockNum != cfBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                     (typeStr=readBasicDataValue()) == null)
    {    //type field not found; set error message
      setErrorMessage("Unable to find response type field in Coefficients" +
                                " Blockette at line " + inTokens.lineno());
      return false;
    }
    Stage stageObj = null;
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if((str=readBlockFieldValue(cfBlkNum,++fieldNum)) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Coefficients " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                ") in Coefficients Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                     "Coefficients Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
    ...
    {    //different stage type set previously
      setErrorMessage("Different transfer type (\"" + typeStr +
                    "\") in same stage in Coefficients Blockette at line " +
                                                         inTokens.lineno());
      return false;
    }
    stageObj.type = typeObj;      //save new transfer/response type
    Unit unitObj;
         //process in-units type value:
    if((str=readBlockFieldValue(cfBlkNum,++fieldNum,true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in Coefficients " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    ...
    if((str=readBlockFieldValue(cfBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in Coefficients " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    ...
    if((str=readBlockFieldValue(cfBlkNum,++fieldNum)) == null)
    {    //number of numerators field not found; set error message
      setErrorMessage("Unable to find number of numerators field in " +
                     "Coefficients Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of numerators field (" + str +
                ") in Coefficients Blockette at line " + inTokens.lineno());
      return false;
    }
         //process # of denominators count (field # is +3 after numerators):
    if((str=readBlockFieldValue(cfBlkNum,fieldNum+3)) == null)
    {    //number of denominators field not found; set error message
      setErrorMessage("Unable to find number of denominators field in " +
                     "Coefficients Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of denominators field (" + str
              + ") in Coefficients Blockette at line " + inTokens.lineno());
      return false;
    }
    BlockFieldSpec bfSpecObj;
    int i;
    CoefficientErrored [] numeratorsArr =
                                      new CoefficientErrored[numNumerators];
    ++fieldNum;                   //move to next field number
    for(i=0; i<numNumerators; ++i)
    {    //for each line of numerators items
              //read and check "B###F##" string; read and discard integer
              // index value; then read 2 floating-point values:
      if((bfSpecObj=readNextBlockFieldNums(cfBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != cfBlkNum || bfSpecObj.fieldNum != fieldNum ||
                                      bfSpecObj.endFieldNum != fieldNum+1 ||
                                            readNextTokenString() == null ||
                        (numeratorsArr[i]=readCoefficientErrored()) == null)
      {
        setErrorMessage("Error parsing numerators data items in " +
                     "Coefficients Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    CoefficientErrored [] denominatorsArr =
                                    new CoefficientErrored[numDenominators];
    fieldNum += 3;                //move to denominators data items field
    for(i=0; i<numDenominators; ++i)
    {    //for each line of denominators items
              //read and check "B###F##" string; read and discard integer
              // index value; then read 2 floating-point values:
      if((bfSpecObj=readNextBlockFieldNums(cfBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != cfBlkNum || bfSpecObj.fieldNum != fieldNum ||
      ...
      {
        setErrorMessage("Error parsing denominators data items in " +
                     "Coefficients Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    {    //no previous filter entered
         //create and configure Coefficients filter:
      final Filter filterObj = new Filter();
      filterObj.coeff_filter(
                      new CoefficientFilter(numeratorsArr,denominatorsArr));
              //create and enter array containing filter item:
      stageObj.filters = new Filter[] { filterObj };
    }
    else
    {    //previous filter entered; append new values
      try
      {
        ...
        coeffFilter.denominator = (CoefficientErrored [])
                          (vec.toArray(new CoefficientErrored[vec.size()]));
      }
      catch(Exception ex)
      {
        setErrorMessage("Internal error:  Exception extending arrays " +
                                         "in 'readCoefficients()':  " + ex);
        return false;
      }
    }
    return true;
  ...
  private boolean readList(BlockFieldSpec initBfSpecObj,boolean dictFlag)
  {
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if(initBfSpecObj.blockNum != rlBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in List " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                        ") in List Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                             "List Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
      ++fieldNum;            //increment number for next field
              //read in next field:
      if((initBfSpecObj=readNextBlockFieldNums(rlBlkNum,fieldNum)) == null)
      {       //error reading next field; set error message
        setErrorMessage("Unable to find in-units field in " +
                             "List Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    if(initBfSpecObj.blockNum != rlBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                     (str=readBasicDataValue(true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in List " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    {    //no previous in-units
      ...
      stageObj.input_units = unitObj;       //enter new units value
         ...
         //process out-units type value:
    if((str=readBlockFieldValue(rlBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in List " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    ...
    if((str=readBlockFieldValue(rlBlkNum,++fieldNum)) == null)
    {    //number of responses field not found; set error message
      setErrorMessage("Unable to find number of responses field in " +
                             "List Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of responses field (" + str +
                        ") in List Blockette at line " + inTokens.lineno());
      return false;
    }
    BlockFieldSpec bfSpecObj;
    ...
    {    //for each line of response items
              //read and check "B###F##" string; read and discard integer
              // index value; then read 5 floating-point values:
      if((bfSpecObj=readNextBlockFieldNums(rlBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != rlBlkNum || bfSpecObj.fieldNum != fieldNum ||
                                      bfSpecObj.endFieldNum != fieldNum+4 ||
//                                            readNextTokenString() == null ||
                                      (rListObj=readRespListItem()) == null)
      {
        setErrorMessage("Error parsing response data items in " +
                             "List Blockette at line " + inTokens.lineno());
        return false;
      }
      ...
      phaseErrArr[i] = rListObj.phaseError; //save phase error value
         ...
         //enter new filter into stage:
    if(stageObj.filters == null || stageObj.filters.length <= 0)
    {    //no previous filter entered; create and configure List filter
    ...
    {    //previous filter entered; append new values
      try
      {
        ...
        listFilterObj.frequency = RespUtils.appendArrays(
                                           listFilterObj.frequency,freqArr);
        listFilterObj.amplitude = RespUtils.appendArrays(
                                            listFilterObj.amplitude,ampArr);
        listFilterObj.amplitude_error = RespUtils.appendArrays(
                                   listFilterObj.amplitude_error,ampErrArr);
        listFilterObj.phase = RespUtils.appendArrays(
                                              listFilterObj.phase,phaseArr);
        listFilterObj.phase_error = RespUtils.appendArrays(
                                     listFilterObj.phase_error,phaseErrArr);
      }
      catch(Exception ex)
      {
    ...
    // the response.
    // @param dictFlag true for dictionary blockette, false for not.
  private boolean readGeneric(BlockFieldSpec initBfSpecObj,
                                                           boolean dictFlag)
  {
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if(initBfSpecObj.blockNum != grBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Generic Response " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
            ") in Generic Response Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                 "Generic Response Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
      ++fieldNum;            //increment number for next field
              //read in next field:
      if((initBfSpecObj=readNextBlockFieldNums(grBlkNum,fieldNum)) == null)
      {       //error reading next field; set error message
        setErrorMessage("Unable to find in-units field in " +
                 "Generic Response Blockette at line " + inTokens.lineno());
        return false;
    ...
    if(initBfSpecObj.blockNum != grBlkNum ||
                                     ...
                                     (str=readBasicDataValue(true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in Generic Response " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    {    //no previous in-units
      ...
      stageObj.input_units = unitObj;       //enter new units value
         ...
         //process out-units type value:
    if((str=readBlockFieldValue(grBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in Generic " +
                         "Response Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    {    //no previous out-units
    ...
    if((str=readBlockFieldValue(grBlkNum,++fieldNum)) == null)
    {    //number of corners field not found; set error message
      setErrorMessage("Unable to find number of corners field in " +
                 "Generic Response Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of corners field (" + str +
            ") in Generic Response Blockette at line " + inTokens.lineno());
      return false;
    }
    for(int i=0; i<numCorners; ++i)
    {    //for each line of corner data; skip line
      if(readNextBlockFieldNums() == null)
      {       //error reading line; set error message
        setErrorMessage("Error reading corner data in " +
                 "Generic Response Blockette at line " + inTokens.lineno());
        return false;
      }
    ...
    // the response.
    // @param dictFlag true for dictionary blockette, false for not.
  private boolean readDecimation(BlockFieldSpec initBfSpecObj,
                                                           boolean dictFlag)
  {
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if(initBfSpecObj.blockNum != dcBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Decimation " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                       "Decimation Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
      ++fieldNum;            //increment number for next field
              //read in next field:
      if((initBfSpecObj=readNextBlockFieldNums(dcBlkNum,fieldNum)) == null)
      {       //error reading next field; set error message
        setErrorMessage("Unable to find input sample rate field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    if(initBfSpecObj.blockNum != dcBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
    {    //unable to find input sample rate field; set error message
      setErrorMessage("Unable to find input sample rate field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse input sample rate field (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    final SamplingImpl samplingImplObj =    // (use scaled-up integer value)
                        new SamplingImpl((int)(sampRate*sampInterval + 0.5),
                            new TimeInterval(sampInterval,UnitImpl.SECOND));
         //process decimation factor field:
    if((str=readBlockFieldValue(dcBlkNum,++fieldNum)) == null)
    {    //unable to find decimation factor field; set error message
      setErrorMessage("Unable to find decimation factor field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse decimation factor field (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
         //process decimation offset field:
    if((str=readBlockFieldValue(dcBlkNum,++fieldNum)) == null)
    {    //unable to find decimation offset field; set error message
      setErrorMessage("Unable to find decimation offset field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse decimation offset field (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
         //process estimated delay field:
    if((str=readBlockFieldValue(dcBlkNum,++fieldNum)) == null)
    {    //unable to find estimated delay field; set error message
      setErrorMessage("Unable to find estimated delay field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse estimated delay field (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
         //create delay-interval object for FISSURES Decimation object:
    final TimeInterval delayIntervalObj =
                                 new TimeInterval(estDelay,UnitImpl.SECOND);
         //process correction applied field:
    if((str=readBlockFieldValue(dcBlkNum,++fieldNum)) == null)
    {    //unable to find correction applied field; set error message
      setErrorMessage("Unable to find correction applied field in " +
                       "Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse correction applied field (" + str +
                  ") in Decimation Blockette at line " + inTokens.lineno());
      return false;
    }
         //create correction-interval object for FISSURES Decimation object:
    final TimeInterval corrIntervalObj =
                              new TimeInterval(corrApplied,UnitImpl.SECOND);

    if(stageObj.the_decimation != null)
    {    //decimation data already entered; set error message
      setErrorMessage("Extra Decimation Blockette for stage at line " +
                                                         inTokens.lineno());
      return false;
    }
         //create and add array containing FISSURES decimation object:
    stageObj.the_decimation = new Decimation[] {
                                  new Decimation(samplingImplObj,deciFactor,
                             deciOffset,delayIntervalObj,corrIntervalObj) };
    return true;
  }
  ...
  private boolean readSensGain(BlockFieldSpec initBfSpecObj,
                                                boolean dictFlag,int seqNum)
  {
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if(initBfSpecObj.blockNum != sgBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Sensitivity/Gain " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      try
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
            ") in Sensitivity/Gain Blockette at line " + inTokens.lineno());
        return false;
      }
         //(seqNum==0 is OK; means sensitivity for entire channel)
      if(seqNum!=0 && seqNum!=curStageSeqNum && seqNum!=curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                 "Sensitivity/Gain Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
      ++fieldNum;            //increment number for next field
              //read in next field:
      if((initBfSpecObj=readNextBlockFieldNums(sgBlkNum,fieldNum)) == null)
      {       //unable to find sensitivity field; set error message
        setErrorMessage("Unable to find sensitivity field in " +
                 "Sensitivity/Gain Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    else      //is a "dictionary" type entry
    {         //stage sequence # should be given in 'segNum' (0 is OK)
      if(seqNum != 0 && seqNum != curStageSeqNum)
      {       //bad stage sequence # (shouldn't happen); set error message
        setErrorMessage("Internal error:  Stage seq# (" + seqNum +
                                      ") out of order in 'readSensGain()'");
        return false;
      }
    }
    if(seqNum > 0 && stageObj == null)
    {    //stage sequence number is not 0 and not new; use latest stage
      final int len;
      if((len=curStagesVec.size()) <= 0)       //get current # of stages
      {     //no current stages (shouldn't happen); set error message
        setErrorMessage("Internal error:  Vector of stages empty " +
    ...
    {
      setErrorMessage("Unable to parse sensitivity field (" + str +
            ") in Sensitivity/Gain Blockette at line " + inTokens.lineno());
      return false;
    }
         //process frequency of sensitivity field:
    if((str=readBlockFieldValue(sgBlkNum,++fieldNum)) == null)
    {    //unable to find frequency of sensitivity field; set error message
      setErrorMessage("Unable to find frequency of sensitivity field in " +
                 "Sensitivity/Gain Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse frequency of sensitivity field (" +
                          str + ") in Sensitivity/Gain Blockette at line " +
                                                         inTokens.lineno());
      return false;
    }
         //process number of calibrations count:
    if((str=readBlockFieldValue(sgBlkNum,++fieldNum)) == null)
    {    //number of calibrations field not found; set error message
      setErrorMessage("Unable to find number of calibrations field in " +
                 "Sensitivity/Gain Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of calibrations field (" +
                          str + ") in Sensitivity/Gain Blockette at line " +
                                                         inTokens.lineno());
      return false;
    }
    for(int i=0; i<numCalibrations; ++i)
    {    //for each line of calibration history data; skip line
      if(readNextBlockFieldNums() == null)
      {       //error reading line; set error message
        setErrorMessage("Error reading calibration history data in " +
                 "Sensitivity/Gain Blockette at line " + inTokens.lineno());
        return false;
      }
      if(!clearToEndOfLine())     //read & discard remaining items on line
    ...
    if(seqNum == 0)
    {    //this is a stage 0 channel sensitivity setting
      if(curSensitivityObj != null)
      {       //response sensitivity already set; set error message
        setErrorMessage("Extra Sensitivity/Gain Blockette in response "+
                                            "at line " + inTokens.lineno());
        return false;
      }
    ...
    {    //not a stage 0 channel sensitivity setting
      if(stageObj.the_gain != null)
      {    //decimation data already entered; set error message
        setErrorMessage("Extra Decimation Blockette for stage at line " +
                                                           inTokens.lineno());
  ...
  private boolean readFIR(BlockFieldSpec initBfSpecObj,boolean dictFlag)
  {
    ...
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if(initBfSpecObj.blockNum != frBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                         (str=readBasicDataValue()) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in FIR " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                         ") in FIR Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1)
      {       //bad stage sequence number; set error message
        setErrorMessage("Stage seq# (" + seqNum + ") out of order in " +
                              "FIR Blockette at line " + inTokens.lineno());
        return false;
      }
         //if new seqNum then create new Stage object and add to list:
      stageObj = createAddNewStageObj(seqNum);
      ...
      if((initBfSpecObj=readNextBlockFieldNums()) != null &&
                                         initBfSpecObj.fieldNum == fieldNum)
      ...
      if(initBfSpecObj == null)
      {  //unable to find symmetry code field; set error message
        setErrorMessage("Unable to find symmetry code field in " +
                              "FIR Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    {    //stage type previously set
      if(!stageObj.type.equals(TransferType.DIGITAL))
      {       //stage type different from previous type
        setErrorMessage("Non-digital filter type already in stage in FIR " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    if(initBfSpecObj.blockNum != frBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                      (symStr=readBasicDataValue()) == null)
    {    //unable to find symmetry code field; set error message
      setErrorMessage("Unable to find symmetry code field in " +
                              "FIR Blockette at line " + inTokens.lineno());
      return false;
    ...
    Unit unitObj;
         //process in-units type value:
    if((str=readBlockFieldValue(frBlkNum,++fieldNum,true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in FIR " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    ...
    if((str=readBlockFieldValue(frBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in FIR " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    ...
    if((str=readBlockFieldValue(frBlkNum,++fieldNum)) == null)
    {    //number of coefficients field not found; set error message
      setErrorMessage("Unable to find number of coefficients field in " +
                           "FIR Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    {
      setErrorMessage("Unable to parse number of coefficients field (" +
                   str + ") in FIR Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    else if(symStr.equals("B"))
    {    //odd number of coefficients, with symmetry
      symFlag = true;
      numNumerators = (numCoefficients*2) - 1;
    }
    else if(symStr.equals("C"))
    {    //even number of coefficients, with symmetry
      symFlag = true;
      numNumerators = numCoefficients * 2;
    }
    else
    {    //unexpected symmetry code; set error message
      setErrorMessage("Invalid symmetry code field (" +
                       symStr + ") in FIR Blockette at line " + symLineNum);
      return false;
    }
    ...
    {    //for each coefficient expected
              //read and check "B###F##" string; read and discard integer
              // index value; then read floating-point value string:
      if((bfSpecObj=readNextBlockFieldNums(frBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != frBlkNum || bfSpecObj.fieldNum != fieldNum ||
                                            readNextTokenString() == null ||
                                        (str=readNextTokenString()) == null)
      {       //error reading field; set error message
        setErrorMessage("Unable to find coefficient field in FIR " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      try
      ...
      {       //error converting string to floating-point value; set message
        setErrorMessage("Error parsing coefficient value (" + str +
                         ") in FIR Blockette at line " + inTokens.lineno());
        return false;
      }
                        //enter coefficient value:
      numeratorsArr[i] = new CoefficientErrored(fVal,(float)0.0);
      if(symFlag && (j=numNumerators-i-1) != i)
      {  //symmetry is active and mirror index is not same value
    ...
    {    //no previous filter entered
      ...
      final Filter filterObj = new Filter();
      filterObj.coeff_filter(
            new CoefficientFilter(numeratorsArr,new CoefficientErrored[0]));
              //create and enter array containing filter item:
      stageObj.filters = new Filter[] { filterObj };
    }
    else
  ...
  private boolean readPolynomial(BlockFieldSpec initBfSpecObj,boolean dictFlag)
  {
    ...
    final String typeStr;
         //check block/field #s and get "Transfer function type" value:
    if(initBfSpecObj.blockNum != rpBlkNum ||
                                       initBfSpecObj.fieldNum != fieldNum ||
                                     (typeStr=readBasicDataValue()) == null)
    {    //type field not found; set error message
      setErrorMessage("Unable to find transfer-function type field in " +
                       "Polynomial Blockette at line " + inTokens.lineno());
      return false;
    }
    Stage stageObj = null;
    String str;
    if(!dictFlag)
    {    //not a "dictionary" type entry; get "Stage sequence number"
      if((str=readBlockFieldValue(rpBlkNum,++fieldNum)) == null)
      {       //seq # field not found; set error message
        setErrorMessage("Unable to find stage seq# in Polynomial " +
                                  "Blockette at line " + inTokens.lineno());
        return false;
      }
      final int seqNum;
      ...
      {
        setErrorMessage("Unable to parse stage seq# (" + str +
                  ") in Polynomial Blockette at line " + inTokens.lineno());
        return false;
      }
      if(seqNum != curStageSeqNum && seqNum != curStageSeqNum+1 &&
                                                                seqNum != 0)
      ...
      if(seqNum != curStageSeqNum ||
                                  (seqNum == 0 && curStagesVec.size() <= 0))
      {  //stage sequence number is new or is very first stage
                   //create new Stage object and add to list:
        stageObj = createAddNewStageObj(seqNum,true);
      }
      ++fieldNum;            //increment number for next field
              //read in next field:
      if((initBfSpecObj=readNextBlockFieldNums(rpBlkNum,fieldNum)) == null)
      {       //error reading next field; set error message
        setErrorMessage("Unable to find in-units field in " +
                       "Polynomial Blockette at line " + inTokens.lineno());
        return false;
    ...
    {    //unrecognized value; set error message
      setErrorMessage("Invalid transfer-function type (\"" + typeStr +
                "\") in Polynomial Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.type != null && !typeObj.equals(stageObj.type))
    {    //different stage type set previously
      setErrorMessage("Different transfer-function type (\"" + typeStr +
                      "\") in same stage in Polynomial Blockette at line " +
                                                         inTokens.lineno());
      return false;
    }
    stageObj.type = typeObj;      //save new transfer/response type
    ...
    if(initBfSpecObj.blockNum != rpBlkNum ||
                                     ...
                                     (str=readBasicDataValue(true)) == null)
    {    //in-units field not found; set error message
      setErrorMessage("Unable to find in-units field in Polynomial " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.input_units == null)
    {    //no previous in-units
      ...
      stageObj.input_units = unitObj;       //enter new units value
         ...
         //process out-units type value:
    if((str=readBlockFieldValue(rpBlkNum,++fieldNum,true)) == null)
    {    //out-units field not found; set error message
      setErrorMessage("Unable to find out-units field in Polynomial " +
                                  "Blockette at line " + inTokens.lineno());
      return false;
    }
    if(stageObj.output_units == null)
    ...
    if((str=readBlockFieldValue(rpBlkNum,++fieldNum)) == null)
    {    //Polynomial Approximation Type field not found; set error message
      setErrorMessage("Unable to find Polynomial Approximation Type field" +
                   " in Polynomial Blockette at line " + inTokens.lineno());
      return false;
    }
    ...
    if((str=readBlockFieldValue(rpBlkNum,++fieldNum)) != null)
      ...
      polyFilterObj.frequency_units = UtilFns.EMPTY_STRING;
    ...
    {    //field found
      ...
      {
        setErrorMessage("Unable to parse Lower Valid Frequency Bound " +
                    "field (" + str + ") in Polynomial Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
    }
    ...
    {    //field found
      ...
      {
        setErrorMessage("Unable to parse Upper Valid Frequency Bound " +
                    "field (" + str + ") in Polynomial Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
    ...
    {    //field found
      ...
      {
        setErrorMessage("Unable to parse Lower Bound of Approximation " +
                    "field (" + str + ") in Polynomial Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
    ...
    {    //field found
      ...
      {
        setErrorMessage("Unable to parse Upper Bound of Approximation " +
                    "field (" + str + ") in Polynomial Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
    ...
    {    //field found
      ...
      {
        setErrorMessage("Unable to parse Maximum Absolute Error " +
                    "field (" + str + ") in Polynomial Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
    ...
    {
      setErrorMessage("Unable to parse number of coefficients field (" +
            str + ") in Polynomial Blockette at line " + inTokens.lineno());
      return false;
    }
    BlockFieldSpec bfSpecObj;
    int i;
    final CoefficientErrored [] coeffErrValsArr =
                                      new CoefficientErrored[numCoefficients];
    ++fieldNum;                   //move to next field number
    for(i=0; i<numCoefficients; ++i)
    {    //for each line of coefficients items
              //read and check "B###F##" string; read and discard integer
              // index value; then read 2 floating-point values:
      if((bfSpecObj=readNextBlockFieldNums(rpBlkNum,fieldNum)) == null ||
         bfSpecObj.blockNum != rpBlkNum || bfSpecObj.fieldNum != fieldNum ||
                                      bfSpecObj.endFieldNum != fieldNum+1 ||
                                            readNextTokenString() == null ||
                        (coeffErrValsArr[i]=readCoefficientErrored()) == null)
      {
        setErrorMessage("Error parsing coefficients data items in " +
                       "Polynomial Blockette at line " + inTokens.lineno());
        return false;
      }
    }
    ...
    polyFilterObj.coeff_err_values = coeffErrValsArr;
         //enter new filter into stage:
    if(stageObj.filters == null || stageObj.filters.length <= 0)
    {    //no previous filter entered; enter polynomial filter
      final Filter filterObj = new Filter();
    ...
    {    //previous filter entered
      setErrorMessage("More than one instance of Polynomial Blockette " +
                                      "found at line " + inTokens.lineno());
      return false;
    }
    return true;
  }
  ...
  {
    RefInfoSpec refInfoObj;
              //read first reference information fields:
    if((refInfoObj=readReferenceInfo(initBfSpecObj)) == null)
    ...
    BlockFieldSpec bfSpecObj;
    while(true)
    {    //for each stage in reference blockette
      if(stageNum != 0)      //allow stage#==0 (for now)
      {  //stage number not zero
        if(stageNum != curStageSeqNum && stageNum != curStageSeqNum+1)
        {     //bad stage sequence number; set error message
          setErrorMessage("Stage seq# (" + stageNum + ") out of order in " +
                        "Reference Blockette at line " + inTokens.lineno());
          return false;
        }
      ...
      for(rCount=0; rCount<numResponses; ++rCount)
      {  //for each response in stage
        if((bfSpecObj=readNextBlockFieldNums()) == null)
        {     //error fetching first "B###F##" item
          setErrorMessage("Unable to read any valid response data in " +
                        "Reference Blockette at line " + inTokens.lineno());
          return false;
        }
        if(stageNum == 0 && bfSpecObj.blockNum != 48)
        {     //stage seq# is 0 but response type is not Sensitivity/Gain
          setErrorMessage("Stage seq# 0 but not Sensitivity/Gain " +
                                       "blockette (" + bfSpecObj.blockNum +
                  ") in Reference Blockette at line " + inTokens.lineno());
          return false;
        }
        switch(bfSpecObj.blockNum)
        {
      ...
      if((refInfoObj=readReferenceInfo(readNextBlockFieldNums())) == null)
        return false;   //if error then return false (err msg already set)
      if(refInfoObj.numStages != totalStageCount)
      {       //#-of-stages value changed; set error message
        setErrorMessage("Non-matching #-of-stages value (" +
                refInfoObj.numStages + ") in Reference Blockette at line " +
                                                         inTokens.lineno());
        return false;
      }
  ...
  {
    String str;
    if(bfSpecObj == null || bfSpecObj.blockNum != 60 ||
              bfSpecObj.fieldNum != 3 || (str=readBasicDataValue()) == null)
    {    //# of stages value not found; set error message
      setErrorMessage("Unable to find # of stages field in Reference " +
                                  "Blockette at line " + inTokens.lineno());
      return null;
    }
    final int numStages;
    try
    {            //convert # of stages string to integer:
      numStages = Integer.parseInt(str);
    }
    catch(NumberFormatException ex)
    {
      setErrorMessage("Unable to parse # of stages (" + str +
                   ") in Reference Blockette at line " + inTokens.lineno());
      return null;
    }
    if(numStages <= 0)
    {    //# of stages value is zero or less; set error message
      setErrorMessage("Bad # of stages value (" + numStages +
                   ") in Reference Blockette at line " + inTokens.lineno());
      return null;
    }
    if((str=readBlockFieldValue(60,4)) == null)
    {    //stage number not found; set error message
      setErrorMessage("Unable to find stage number in Reference " +
                                  "Blockette at line " + inTokens.lineno());
      return null;
    }
    final int stageNum;
    try
    {            //convert stage number string to integer:
      stageNum = Integer.parseInt(str);
    }
    catch(NumberFormatException ex)
    {
      setErrorMessage("Unable to parse stage number (" + str +
                   ") in Reference Blockette at line " + inTokens.lineno());
      return null;
    }
    if((str=readBlockFieldValue(60,5)) == null)
    {    //# of responses field not found; set error message
      setErrorMessage("Unable to find # of responses field in Reference " +
                                 "Blockette at line " + inTokens.lineno());
      return null;
    }
    final int numResps;
    try
    {            //convert # of responses value string to integer:
      numResps = Integer.parseInt(str);
    }
    catch(NumberFormatException ex)
    {
      setErrorMessage("Unable to parse # of responses value (" + str +
                   ") in Reference Blockette at line " + inTokens.lineno());
      return null;
    }
    if(numResps <= 0)
    {    //# of responses value is zero or less; set error message
      setErrorMessage("Bad # of stages value (" + numResps +
                   ") in Reference Blockette at line " + inTokens.lineno());
      return null;
    }
    ...
    /**
     * @return the given block/field value or null if the next item cannot be
     * be found, its numbers do not match, or if an error occurs.
     * @param blockNum block number
     * @param fieldNum field number
     * @param lineFlag if true then all the rest of the tokens on the
  ...
  {
    BlockFieldSpec bfSpecObj;
    if((bfSpecObj=readNextBlockFieldNums()) == null)  //fetch "B###F##"
    ...
    /**
     * @return the given block/field number item and returns its
     * associated value.  Returns null if the next item cannot be
     * be found, its numbers do not match, or if an error occurs.
     * @param blockNum block number
     * @param fieldNum field number
     */
  public String readBlockFieldValue(int blockNum,int fieldNum)
  {
    ...
    /**
     * @return the block and field numbers of the next
     * "B###F##..." string or null if valid string not found or error.
     * Any leading blank for comment lines are skipped, but the next
     * word token must be a valid "B###F##..." string or null is returned.
     */
  public BlockFieldSpec readNextBlockFieldNums()
  {
     ...
     * @param blockNum block number to find.
     * @param fieldNum field number to find.
     * @return the block and field numbers of the for requested block,
     * or null if valid string not found or error.
     */
   public BlockFieldSpec readNextBlockFieldNums(int blockNum,int fieldNum)
   {
     BlockFieldSpec bfSpecObj;
     while(true)
     {   //for each block to be scanned; fetch next block
       if((bfSpecObj=readNextBlockFieldNums()) == null)
         return null;        //if not found then return error
       if(bfSpecObj.blockNum == blockNum)
       {      //block number matches; check field number
    ...
    /**
     * Scans until a block matching one of the specified set of values
     * is found.
     * @param blockNum1 first block number to find.
     * @param fieldNum1 first field number to find.
     * @param blockNum2 second block number to find.
     * @param fieldNum2 second field number to find.
     * @return The block and field numbers for the requested block,
     * or null if valid string not found or error.
     */
   public BlockFieldSpec readNextBlockFieldNums(int blockNum1,int fieldNum1,
                                                int blockNum2,int fieldNum2)
   {
     BlockFieldSpec bfSpecObj;
     while(true)
     {   //for each block to be scanned; fetch next block
       if((bfSpecObj=readNextBlockFieldNums()) == null)
         return null;        //if not found then return error
       if(bfSpecObj.blockNum == blockNum1)
       {      //block number matches; check field number
         if(bfSpecObj.fieldNum == fieldNum1)
           return bfSpecObj;      //if field # matches then return object
       }
       if(bfSpecObj.blockNum == blockNum2)
       {      //block number matches; check field number
         if(bfSpecObj.fieldNum == fieldNum2)
           return bfSpecObj;      //if field # matches then return object
       }
       else if(bfSpecObj.blockNum > blockNum1 &&
                                             bfSpecObj.blockNum > blockNum2)
       { //beyond both block numbers
    ...
    /**
     * @return the next token or null if token not found or
     * error.
     */
  public String readNextTokenString()
  {
    ...
    /**
     * @return the token after the next "xxx:" token or
     * null if token not found or error.
     * @param lineFlag if true then all the rest of the tokens on the
     * line are also returned; if false then only the next token is returned
     * and the rest on the line are read in and discarded.
     */
  public String readBasicDataValue(boolean lineFlag)
  {
    ...
    try
    {
      ...
      {       //for each word token found; check for "xxx:"
        if(inTokens.getNonNullTokenString().endsWith(":"))
        {     //leading "xxx:" token found
          if(inTokens.nextToken() == RespTokenizer.TT_WORD)
          {   //next word token found
            str = inTokens.getTokenString();     //get token string
                        //read in any remaining word tokens on line:
            while(inTokens.nextToken() == RespTokenizer.TT_WORD)
            {      //for each token remaining on line
    ...
    /**
     * @return the token after the next "xxx:" token or
     * null if token not found or error.  The single token is returned
     * and the rest on the line are read in and discarded.
     */
  public String readBasicDataValue()
  {
    ...
    /**
     * @return four floating-point values; returns null if
     * unexpected input or error.
     */
  public ComplexNumberErrored readComplexNumberErrored()
  {
    ...
    int tType;
    for(int p=0; p<4; ++p)
    {    //for each floating-point value converted
      try
      {                      //skip any blank (or comment) lines:
        while((tType=inTokens.nextToken()) == RespTokenizer.TT_EOL);
      }
    ...
    return new ComplexNumberErrored(        //create and return object
                           floatArr[0],floatArr[2],floatArr[1],floatArr[3]);
  }

    /**
     * @return two floating-point values; returns null if
     * unexpected input or error.
     */
  public CoefficientErrored readCoefficientErrored()
  {
     ...
     * @return A new 'RespListItem' object, or null if unexpected input
     * or error.
     */
  public RespListItem readRespListItem()
  {
    try
    {
      ...
      {  //for each floating-point value converted
        ...
        if((tType=inTokens.nextToken()) == RespTokenizer.TT_EOL)
      ...
      return new RespListItem(floatArr[p-4],floatArr[p-3],floatArr[p-2],
                                                 floatArr[p-1],floatArr[p]);
    }
    catch(IOException ex)
    {    //error reading token; set error message
      setErrorMessage("Error reading from input file: " + ex);
     ...
     * @return true if successful, false if an I/O error is detected (in
     * which case an error message will be set).
     */
  ...
  {
    final int len;
    if(str == null || (len=str.length()) < 7 || len > 10 ||
                                                       str.charAt(0) != 'B')
    {    //string data not valid or does not start with 'B'
      return null;
    }
    ...
    if(len > 7)
    {    //more data available; must have end field number
      if(str.charAt(7) != '-' || len < 10)  //if no dash or too short then
        return null;                        //return error
      try
      {            //parse end field number:
  ...
  protected void setInfoMessage(String str)
  {
    ...
    /**
     * @return true if an info message has been entered.  The info message
  ...
  class RespListItem
  {
    ...
    public RespListItem(float freq,float amp,float ampError,float phase,
                                                           float phaseError)
    {
      this.freq = freq;
      this.amp = amp;
